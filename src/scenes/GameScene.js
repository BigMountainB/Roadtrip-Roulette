...
// offsetting the UI camera HUD_OFFSET_X.  Namespace import so these read the
...
export class GameScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    this._resumeSnapshot = data?.resumeSnapshot ?? null;   // full portable-save state
    ...
    this._dailyEndPos = null;     // route position at which the stage completes
    this._dailyDone   = false;    // guards the one-shot stage-complete trigger
    ...
    // Menu clear `liveRun` first, so those deliberate returns still show the
    // title rather than yanking the player back into a dead run.
    ...
    if (data?.resumeLiveSnapshot && typeof data.resumeLiveSnapshot === 'object'
      ...
      // you" modal, this was a deliberate continue.
      this._resumeLive = { snap: data.resumeLiveSnapshot, ts: Date.now() };
      this._resumeLiveExplicit = true;
    } else if (!this._resumeFromStop && this._resumeFromPosition == null
        && !this._skipTitle && !this._dailyStage && !this._missionConfig) {
      try {
        const _lrSave = this.registry.get('save');
        const lr = _lrSave?.get?.('liveRun');
        // Crash marker (set by main.js's error handlers).  A genuine JS crash
  ...
  _doCreate() {
    ...
    // Custom RESUME path — a rest-stop exit / crash respawn re-runs init() with
    // NO modal, so restore the persisted sandbox loadout (set on custom start)
    // so the run keeps its No-damage / No-police / accessory choices instead
    // of reverting.  A FRESH custom run's modal overwrites these right after
    // init(), so this never fights the modal flow.  Non-custom modes skip it.
    if (Difficulty.mode?.() === 'custom') {
      const _cl = this.registry?.get?.('customLoadout');
      if (_cl) {
        this._customFlags = { noNpcDamage: !!_cl.noNpcDamage, noPolice: !!_cl.noPolice };
        if (_cl.accessories) this._customStartAccessories = _cl.accessories;
      }
    }
    ...
    // shake scale (0–1).  hudHidden: "Hide HUD" toggle.
    {
      ...
      // "Hide HUD" toggle was replaced by the Customize Controls editor — the
      // HUD now always shows in gameplay (force false so any old saved
      // settings.hud=false doesn't leave a profile stuck with a blank HUD).
    ...
    // cadence through the last half-mile before any speed trap.  Driven each
    // frame by _updateRadar(); both elements hidden until a trap is near.
    this._hasRadar = !!this.registry?.get?.('save')?.get?.('radarDetector', false);
    // Custom resume: the sandbox radar toggle (persisted in customLoadout)
    // wins over global ownership so a custom run keeps its radar state.
    if (Difficulty.mode?.() === 'custom') {
      const _cl = this.registry?.get?.('customLoadout');
      if (_cl) this._hasRadar = !!_cl.radar;
    }
    this._radarGfx = this.add.graphics().setDepth(19).setVisible(false);
    this._hudObjects.push(this._radarGfx);
    this._radarLabel = this.add.text(40, 116, 'SPEED TRAP', {
      fontSize: '11px', fontFamily: IMPACT, color: '#FF5555',
      stroke: '#000', strokeThickness: 3,
    }).setOrigin(0.5, 0).setDepth(19).setVisible(false);
    this._hudObjects.push(this._radarLabel);
    this._radarBeepTimer = 0;     // counts up to the next blip
    this._radarBlinkOn   = false; // dashboard-light blink phase
    ...
    // Player car: the real per-vehicle rear art (`spriteBack`).  The old orange
    // procedural fallback was deleted 2026-06-23 per user — never show it.  The
    // car art is tiny, so if it hasn't finished loading the instant the sprite
    // is created we just mount an INVISIBLE 1×1 and swap the real texture in the
    // moment it's available (see _ensurePlayerArtReady, called each frame).  The
    // sprite always exists so plate-anchor / lean / sizing have a valid target.
    ...
    // Per-car OUTLINE rim — a slightly larger, tinted copy of each car drawn
    // just BEHIND it so a thin edge peeks around the body, popping every NPC
    // off the grey road regardless of its color.  Centred origin so the
    // enlarged copy rims evenly; positioned / sized / tinted in _renderVehicles.
    ...
    for (let i = 0; i < 36; i++) {
      ...
      const ol = this.add.image(0, 0, 'npc_car_white')
        .setOrigin(0.5, 0.5)
        .setDepth(7.9)
        .setVisible(false);
      this._carOutlinePool.push(ol);
    ...
    // Fog light-glow layer — solid colored lit lights (tail / headlight /
    // cop bar) shown in the fog.  NORMAL blend (ADD washes out on bright
    // daytime fog).  Depth 10.02 = ABOVE the player car (9.95) AND the fog
    // veil (10) so the lights punch through the haze instead of getting
    ...
    // scene pool).  The alcohol ghost offset scales with on-screen size, so
    // only the nearest/biggest buildings/trees/signs show a readable double;
    // far scenery's ghost lands on top of itself.  40 covers the near cluster
    // without doubling the hottest render path's memory or draw calls.
    this._sceneGhostPool = [];
    for (let i = 0; i < 40; i++) {
      this._sceneGhostPool.push(this.add.image(0, 0, 'codex_bellevue_skyline')
      ...
      () => {
        ...
        this._buildConfirmPopup(
          ...
          () => {
            ...
            // Drop the mid-drive autosave too — without this the fresh
            // scene.start auto-resumes the run we're trying to abandon
            // ("sorry, we lost you…" on every Start Over).
            _save?.set?.('liveRun', null);
            this._resumeLive         = null;
    ...
    const checkpointBtn = this._buildPauseButton(
      ...
      0x39A8FF,
      async () => {
        // Resume the LATEST checkpoint directly — no copy/paste code needed.
        // Order: cloud save (cross-device, keyed by playerId) → local
        // lastRestStop → finally the manual code modal if there's nothing to
        // resume (e.g. resuming someone else's code on a fresh device).
        if (this._cpResumeBusy) return;
        this._cpResumeBusy = true;
        try {
          const save = this.registry?.get?.('save');
          // Gather candidates: cloud (cross-device) + local exact-spot save,
          // and pick the NEWEST.  Both can be either a manual exact-spot save
          // (has position, no rest-stop id) or a rest-stop snapshot (has id).
          let cloudSnap = null, cloudTs = 0;
          try {
            const cloud = await CloudSave.get(save?.activePlayerId);
            if (cloud?.snapshot) { cloudSnap = cloud.snapshot; cloudTs = Date.parse(cloud.updatedAt || '') || 0; }
          } catch (_) { /* offline → local fallback */ }
          const liveRun  = save?.get?.('liveRun');
          const liveSnap = (liveRun?.snap && typeof liveRun.snap.position === 'number') ? liveRun.snap : null;
          const liveTs   = liveRun?.ts || 0;
          let snap = null;
          if (liveSnap && (!cloudSnap || liveTs >= cloudTs)) snap = liveSnap;   // newest local
          else if (cloudSnap)                                snap = cloudSnap;  // cloud (or only option)
          else { const rest = save?.get?.('lastRestStop'); if (rest?.id) snap = rest; }
          if (snap) {
            const isRestStop = snap.id && REST_STOPS.some(r => r.id === snap.id);
            if (isRestStop) this._resumeFromSavedSnapshot(snap);   // jump to that rest stop
            else            this._resumeFromLiveSnapshot(snap);    // exact-spot continue
            return;
          }
          // Nothing to resume → let them type / paste a code.
          this._promptForCode('');
        } finally {
          this._cpResumeBusy = false;
        }
      },
    );
    this._pauseObjects.push(checkpointBtn.bg, checkpointBtn.txt);

    // ── Desktop-only: a button into the iPhone menu ──────────────────────
    // Phones reach the menu by rotating to portrait; desktop can't rotate, so
    // this opens it directly.  window.__phoneMenu.open() keeps the game paused
    // and shows the menu; the menu's "enter gameplay" button calls .close()
    // to resume.  Sits centered just above the START OVER / FROM CHECKPOINT
    // row.  Created only on desktop so phones are completely unaffected.
    if (window.matchMedia?.('(hover: hover) and (pointer: fine)')?.matches) {
      const phoneBtn = this._buildPauseButton(
        SCREEN_W / 2, buttonY - 46, 240, 34, 'iPHONE MENU',
        0x39FF8A,
        () => { window.__phoneMenu?.open?.(); },
      );
      this._pauseObjects.push(phoneBtn.bg, phoneBtn.txt);
    }
    ...
    if (this._resumeLive) {
      ...
      const segWorld = ROUTE_SEGS * SEG_LENGTH;
      this.player.position = Math.max(0, lr.snap.position || 0);
      ...
      const cpNear = [...CHECKPOINTS].reverse().find(c => c.t <= tNow);
      this._lastCheckpoint = {
        name:      cpNear?.name ?? 'West Seattle',
        position:  cpNear ? cpNear.t * segWorld : this.player.position,
        scoreAtCP: this.score,
      };
      this._partyClockSecMax = Difficulty.partyClockSec?.() ?? 2400;
      this._partyClockSec    = (typeof lr.snap.partyClockSec === 'number')
        ? Math.max(0, Math.min(this._partyClockSecMax, lr.snap.partyClockSec))
        : this._partyClockSecMax;
      if (typeof lr.snap.gameTime === 'number') this.gameTime = Math.max(0, lr.snap.gameTime);
      this._awaitingStart    = false;
      this._introDone        = true;
      this._steerLockUntilTap = true;
      // Show the "sorry, we lost you…" modal only for an UNEXPECTED boot resume
      // (iOS discard).  A deliberate Save → From Checkpoint continue skips it.
      this._awaitingResumeOk  = !this._resumeLiveExplicit;
      ...
      const ds     = this._dailyStage;
      const cpFrom = CHECKPOINTS.find(c => c.name === ds.startCity);
      const cpTo   = CHECKPOINTS.find(c => c.name === ds.endCity);
      const startT = cpFrom?.t ?? 0;
      this.player.position = startT * (ROUTE_SEGS * SEG_LENGTH);
      this._odometer       = startT * TOTAL_ROUTE_MILES;
      this._dailyEndPos    = (cpTo?.t ?? 1) * (ROUTE_SEGS * SEG_LENGTH);
      this._dailyDone      = false;
      this._passedCheckpoints = new Set(
        CHECKPOINTS.filter(c => c.t <= startT).map(c => c.name));
      this._lastCheckpoint = {
        name: cpFrom?.name ?? 'West Seattle',
        position: this.player.position, scoreAtCP: 0,
      };
      this._partyClockSec    = Difficulty.partyClockSec?.() ?? 2400;
      ...
      this._dailyTracker = {
        ...
        barrierScrape:   false,       // hit a pole/fence/tree/rail
        offroad:         false,       // crossed onto the shoulder/dirt
        copsKilled:      0,           // cruisers taken down (collision)
      };
      this._dailyResult = null;
      // Car drives straight until the first tap, like the resume paths.
      this._steerLockUntilTap = true;
      ...
      if (rs) {
        ...
        // Rest-stop resume KEEPS your wanted level, minus the −1 a stop
        // naturally shaves off (already applied in resumeStars; 5★ is immune).
        // Paying the clear-heat service makes resumeStars arrive as 0 (full
        // wipe).  Active cop CARS are cleared below so you don't exit into an
        // instant swarm — with stars still set, CopSystem re-spawns a pursuit
        // once a cop spots you.  Bust-strike counters reset for a fresh slate.
        this.cops.stars         = Math.max(0, Math.min(5, this._resumeStars ?? 0));
        ...
        if (buys) {
          ...
          if (typeof buys.addGasMi === 'number' && this.player) {
            ...
            if (buys.addGasMi > 0) this._strandedShown = false;
          }
    ...
    // ── Decoupled-width centering (Task 4) ──────────────────────────────
    // The canvas is widened to WORLD_W (≥ the 800 design) by applyOrientation()
    // so scenery fills the phone's full width.  Default cameras.add() is
    // transparent — do NOT setBackgroundColor here or it paints over the main
    // camera's world.
    // Full-canvas UI camera, scrolled −HUD_OFFSET_X to center the 800-wide HUD
    // (set in _applyDecoupledCameras).  Full-canvas (not a clipped 800 viewport)
    // so Customize-Controls can place HUD items anywhere on the widened screen.
    this._uiCam = this.cameras.add(0, 0, C.WORLD_W, SCREEN_H);
    this.cameras.main.ignore(this._hudObjects);
    this._uiCam.ignore(this._worldObjects);
    this._applyDecoupledCameras();
    // Re-center whenever the canvas is resized to the device aspect (boot rAF /
    // rotation).  main.js's applyOrientation() calls setGameSize AFTER updating
    // HUD_OFFSET_X, and setGameSize fires this scene 'resize' — so we re-read
    ...
    // background (and possibly discard) the tab — snapshot the run right
    // then.  Bound once on the reused scene instance.
    if (!this._autosaveBound) {
      this._autosaveBound = true;
      const flush = () => this._autosaveRun();
      this._onVisAutosave = () => { if (document.hidden) flush(); };
      document.addEventListener('visibilitychange', this._onVisAutosave);
   ...
   *  canvas.  The WORLD is drawn centered at scene-x 400, so the main camera
   *  scrolls −HUD_OFFSET_X to land that at the canvas center (the full-screen
   *  overlay/vignette live in _worldObjects, so they shift with it).  The HUD
   *  keeps its 800-wide layout, centered by the UI camera's viewport inset — no
   *  per-button repositioning, so the title/HUD can't drift.  No-op when
   *  WORLD_W===800 (HUD_OFFSET_X=0). */
  _applyDecoupledCameras() {
    try {
      // Source of truth = the LIVE Phaser canvas width.  A dev-server HMR /
      // reload can reset the module-level HUD_OFFSET_X back to 0 while the
      // canvas keeps its widened size — that desync is what slid the HUD off to
      // the LEFT (under the notch) mid-edit.  If the bindings have drifted from
      // the real canvas, re-derive them.  No-op in production (no HMR) and cheap
      // when already in sync.
      const gw = this.scale?.gameSize?.width;
      if (gw && Math.abs((C.HUD_OFFSET_X * 2 + SCREEN_W) - gw) > 0.5) {
        C.setWorldWidth?.(gw);
      }
      this.cameras.main.setSize(C.WORLD_W, SCREEN_H);
      this.cameras.main.setScroll(-C.HUD_OFFSET_X, 0);
      if (this._uiCam) {
        // Full-canvas viewport + matching scroll: HUD stays centered, but items
        // can be dragged out into the widened side areas (no 800px clip).
        this._uiCam.setViewport(0, 0, C.WORLD_W, SCREEN_H);
        this._uiCam.setScroll(-C.HUD_OFFSET_X, 0);
      }
    } catch (_) {}
  }
  ...
  _setupTouch() {
    ...
    const overViceBar = (p) => {
      ...
      for (const h of hits) {
        if (px >= h.x && px <= h.x + h.w && p.y >= h.y && p.y <= h.y + h.h) return true;
      }
      return false;
    };
    ...
    this.input.on('pointerdown', (p) => {
      ...
      // disguise) lands where the player actually sees the control.  When not
      // decoupled (HUD_OFFSET_X=0) sx === p.x, so this is a no-op.
      ...
      // Default top band (y<64) PLUS any button the player has MOVED out of it
      // via Customize Controls (live boxes published in _applyControlLayout).
      const overTopButtons = p.y < 64 || (this._topRowButtons || []).some(b =>
        b.lbl && b._lsz != null
        && sx >= b._lx && sx <= b._lx + b._lsz
        && p.y >= b._ly && p.y <= b._ly + b._lsz);
      // Weapon cells — exclude each weapon's LIVE box (they're now placed
      // independently) so a weapon tap never also veers the car.
      const wcb = this._weaponCellBounds;
      const overWeaponCol = !!wcb && Object.values(wcb).some(b =>
        sx >= b.x && sx <= b.x + b.w && p.y >= b.y && p.y <= b.y + b.h);
      // Standalone Disguise button (opposite edge, just under Mute) —
      // its live bounds are published each frame by _drawF12Inventory.
      // It sits inside the classic-mode steer band, so exclude it like
      // the weapon column or a tap to go dark would also veer the car.
      const db = this._disguiseHitBounds;
      const overDisguise = !!db
        && sx >= db.x && sx <= db.x + db.w
        && p.y >= db.y && p.y <= db.y + db.h;
      // GAS/BRAKE pedals, raised off the bottom edge — their live bounds
      // are published by _buildHUD / _applyPedalHandedness.  They now sit
      // mid-screen inside the classic steer band, so exclude them too (a
      // 6-px pad for finger slop) or a pedal tap would also veer the car.
      const overPedals = (this._pedalHitZones ?? []).some(pb =>
        sx >= pb.x - 6 && sx <= pb.x + pb.w + 6
        && p.y >= pb.y - 6 && p.y <= pb.y + pb.h + 6);
      // Rear-view mirror — touch-and-HOLD to zoom it to _mirrorHoldZoom×.  Hit-test
      // the RESTING (base) bounds so the press target doesn't move as it
      // grows; release zooms back (handled in the global pointerup).  The
      // base mirror sits entirely inside the top-button band (y < 64), so
      // overTopButtons already suppresses steering on this press.
      const mb0 = this._mirrorBaseBounds;
      const overMirror = !!mb0
        && sx >= mb0.x && sx <= mb0.x + mb0.w
        && p.y >= mb0.y && p.y <= mb0.y + mb0.h;
      // Wiper (only present in rain/snow) — exclude its live box so a tap on it
      // doesn't also steer.  Gated on visible so there's no dead zone in clear
      // weather (when the button isn't on screen).
      const wb = this._wiperLiveBounds;
  ...
  _steeringMode() {
    ...
    if (m === 'tap') m = 'flappy';
    if (!m) {
      // Default: 'default' (adaptive — steering shifts with the weather).
      // Existing players with legacy `tiltSteerEnabled` keep tilt.
      m = this.registry?.get?.('tiltSteerEnabled') ? 'tilt' : 'default';
    }
  ...
  /** Persistent on-screen cue during the snow steering zone — the snow→tilt
   *  switch was silent, so a 'default' player whose scheme flipped to TILT kept
   *  TAPPING (ignored in tilt mode) and felt like "nothing registers".  Shows:
   *    • "📱 SNOW — TILT TO STEER" when tilt is the live scheme, OR
   *    • "❄️ SNOW — SLIPPERY"      when tilt is unavailable (no-trap: digital
   *      stays drivable, see _tiltCoax in _updatePhysics).
   *  Lazily creates the HUD text + registers it with the UI camera. */
  _updateSnowSteerCue() {
    ...
    const cue = this._snowCueText;
    if (!active) { if (cue.visible) cue.setVisible(false); return; }
    const mode = this._activeSteeringMode?.();
  ...
  /** Effective steering mode for INPUT this frame, resolved from the player's
   *  pick in _steeringMode():
   *    'classic' THUMBS / 'flappy' TAP / 'tilt'  → LOCKED to that scheme for
   *      the whole drive (weather never switches a deliberate choice).  'tilt'
   *      needs the sensor attached; without permission it falls back to L/R.
   *    'default'  → the regular adaptive gameplay that switches by WEATHER:
   *      TILT through the snow when the sensor's attached, classic L/R
   *      otherwise.
   *  The Vantage crosswind is NOT a mode switch — it's a leftward pull applied
   *  as a physics force in _updatePhysics, so it layers on top of any scheme.
   *  NOTE: never returns 'default' — it resolves to the concrete scheme.
  ...
  _activeSteeringMode() {
    ...
    //     long-documented "wind→tap" half that was never wired in.
    //   • SNOW          → TILT when the sensor's attached.
  ...
  _setSteeringMode(mode, onDone) {
    ...
    const save = this.registry?.get?.('save');
    if (mode === 'default') {
      // Adaptive "regular gameplay" — classic baseline that auto-switches to
      // TILT in the snow (the weather-based tilt prefetch attaches the sensor
      // when weather is on) plus the Vantage wind pull.  Persist the pick
      // WITHOUT tearing down an attached tilt listener — snow needs it — and
      // without prompting for new permission here.
      this.registry?.set?.('steeringMode', 'default');
      this.registry?.set?.('titleThumbsPick', 'default');
      save?.setMode?.('default');
  ...
  update(time, delta) {
    ...
    // Player car art loads via BootScene; if the sprite was created before it
    ...
    // Controls editor (Settings → Customize Controls).  Armed in portrait;
    // enters once the scene is running in landscape — from the title OR mid-run.
    // While editing, the sim is frozen and we just paint the frozen world + HUD
    // so the controls can be dragged.
    if (this._editorArmed) {
      this._editorArmed = false;
      this._enterControlsEditor();
    }
    ...
    // Live-resume hold: the run is fully restored and frozen behind the
    // "sorry, we lost you…" modal.  Paint the frozen world + HUD until the
    // player taps OK (which clears _awaitingResumeOk); the car then coasts
    // straight until the first steer input, like every other resume path.
    if (this._awaitingResumeOk) {
      this._renderFrame();
      this._renderHUD();
      return;
    }
    ...
    // ── Terminal freeze — the run is ending ────────────────────────────
    ...
    // collision passes that follow it in update() are skipped by the return.
    ...
    // Mid-drive autosave — persist the live run every few seconds so an
    // unexpected iOS reload resumes here instead of losing the trip.  The
    // pagehide/visibilitychange listeners (set in create) catch the discard
    // moment itself; this covers a hard crash with no lifecycle event.
    ...
    // (the switch was previously silent, so taps in tilt mode "did nothing").
    this._updateSnowSteerCue?.();
    ...
    if (this._customFlags?.noPolice) {
      ...
      this.cops.pitCount      = 0;
      this.cops.arrestPending = false;
      // "No police" = no speed traps, no civil stops, no tickets either.  The
      // friend's "speed trap ahead" texts still fire (harmless flavor — they
      // can cry wolf), but nothing here ever actually pulls you over or fines
      // you.  Drop any in-flight trap state defensively (covers a save resumed
      // mid-stop or the flag being toggled on partway through a pursuit).
      this._trapPursuitActive = false;
      this._trapComplyTimer   = 0;
      this._trapStopping      = false;
      this._trapStopHeld      = false;
      ...
      if (segs?.length) {
        ...
        for (let n = 1; n <= 60; n++) {
          ...
          for (const sp of seg.sprites) {
            ...
            // IMPORTANT: only mark the encounter `triggered` (consumed) when it
            // actually RESOLVES — acts, or is cleanly slipped past.  The one
            // no-op case (a parked trap you blow past at 0★ while an EARLIER
            // civil stop is still active) is left un-consumed so it isn't
            ...
            if (speeding || oncoming) {
              if (this.cops.stars >= 1) {
                ...
                // Driving cop at 0★ → ambient, no reaction; consume so it
                // doesn't re-scan every frame.
                sp.triggered = true;
              }
              // else: parked trap, 0★, but a civil stop is already active →
              // leave UN-consumed (the fix); it can still act once that clears.
            } else if (isParkedTrap) {
              sp.triggered = true;
              this._showPopup('Slipped past the trap.', '#88FF88');
            } else {
              // Driving cop, not speeding → ambient; consume.
              sp.triggered = true;
            }
          }
        }
    ...
    // ── Daily Challenge stage end ────────────────────────────────────
    // A daily stage finishes at its TARGET CITY instead of driving to
    // Pullman.  Objective evaluation + the attempt-decay reward land in the
    // next increment; for now, reaching the end city completes the stage and
    // returns to the title.  Guarded (_dailyDone) so it fires exactly once,
    // and only in daily-stage mode — normal runs never enter this branch.
    if (this._dailyStage && !this._dailyDone
        && this.player.position >= (this._dailyEndPos ?? Infinity)) {
      ...
      if (grade.pass) {
        ...
        this._showPopup(
          `✓ CHALLENGE PASSED\n${this._dailyStage.endCity}${why}\n+$${reward.toLocaleString()}`,
          '#39FF8A');
      } else {
        this._showPopup(
          `✗ CHALLENGE FAILED\n${this._dailyStage.endCity}\n${grade.reason || 'Objective not met'}`,
          '#FF5555');
      }
      this.time.delayedCall(2800, () => { try { this.scene.start('Game', {}); } catch (_) {} });
      return;
    }
    for (const cp of CHECKPOINTS) {
      ...
      if (progress >= cp.t) {
        ...
        if (cp.isFinish && !this._gameFinished) {
          ...
          if (onTime) {
            ...
            finishLines.push('😞 TOO LATE — No bonus.');
            finishColor = '#FF6622';
          }
          ...
          // party.  Otherwise she found someone else.
          {
            ...
            if (!_gGone && _gTexts > 0) {
              ...
              finishLines.push(`💕 They're waiting at the party! +$${GIRL_PARTY_BONUS.toLocaleString()}`);
            } else if (_gGone) {
              finishLines.push('💔 They found someone else.');
            }
          }
          // One combined finish popup (made-it/too-late + Crush payoff).
          if (finishLines.length) this._showPopup(finishLines.join('\n'), finishColor);
  ...
  _applySinkCrop() {
    ...
    const shift = this.playerSprite.displayHeight * prog;
    this.playerSprite.y += shift;
    // Sink the license plate WITH the car — it's a separate overlay anchored
    // ~51% up the sprite, positioned before this crop runs, so on its own it
    // floats in place.  Move it down by the same amount, and hide it once the
    // waterline rises past its anchor (~half sink) so it submerges cleanly.
    const plateSubmerged = prog >= 0.5;
    for (const pl of [this._rearPlateImg, this._rearPlate]) {
      if (!pl || pl.visible === false) continue;
      if (plateSubmerged) pl.setVisible(false);
      else pl.y += shift;
    }
  }
  ...
  _updatePlayer(dt, phys) {
    ...
    // ramp just below (loads in on press, unwinds on release); the lateral-
    // velocity settle farther down smooths it further.  Tilt is analog and
    // flappy stays instant.
    ...
    // a beat and unwinds slowly on release instead of snapping.  Only the
    // digital L/R modes (classic + DEFAULT-in-dry, both report 'classic')
    // ramp — tilt is already analog and flappy wants its tight one-tap
    // swing, so both pass through (kept synced so an adaptive mode switch
    // doesn't jump).  Rates are per-second lerp toward target; LOWER = heavier.
    if (_mode === 'classic') {
      // Engage is intentionally SLOW so a quick tap only builds a little
      // lateral (≈ a quarter lane, not half); a sustained hold still reaches
      // full TURN_SPEED, so max steer authority — and the drunk beer-gravity
      // counter-steer balance — is untouched.  Release is FAST so the car
      // settles back toward center quickly and drifts less after a tap.
      const STEER_RAMP_ENGAGE  = 3.0;   // ~0.33 s to load into a turn (smaller per-tap move)
      const STEER_RAMP_RELEASE = 5.0;   // ~0.20 s to unwind back to center (less drift)
      const prev = this._steerRamp ?? 0;
      const rate = (steerIn === 0) ? STEER_RAMP_RELEASE : STEER_RAMP_ENGAGE;
      this._steerRamp = prev + (steerIn - prev) * Math.min(1, rate * dt);
      steerIn = this._steerRamp;
    } else {
      this._steerRamp = steerIn;
    }
    ...
    // ── Vantage crosswind ───────────────────────────────────────────
    // A LEFTWARD pull as strong as a HELD LEFT ARROW (full -1) — it feels
    // like the left arrow is stuck.  It bites to full over the FIRST MILE
    // of the zone (131->132), holds full to 177, then eases out by 183.
    // This pull is DECOUPLED from _windStrength (which paces the tree-sway
    // + tumbleweed VISUALS over the original 6-mile build) so the steering
    // bites hard almost immediately while the scenery still builds in
    // gradually.  Steering RIGHT completely overtakes it (full right
    // turning power); the pull only applies when the player ISN'T actively
    // steering right (so doing nothing drifts hard left, and holding left
    // just pins it).  Applied AFTER player input + alcohol, and NOT flipped
    // by drunk invertSteering — the wind blows the same way no matter how
    // scrambled the controls are.
    let _windPull = 0;
    if (this._windStrength(_mileForSnow) > 0) {
      const _m = _mileForSnow;
      _windPull = _m < 132  ? (_m - 131)                    // ramp to full over the first mile
    ...
    // NO-TRAP: the snow WANDER (and the digital oversensitivity below) exist to
    // coax players onto tilt.  If tilt isn't actually available on this device
    // (permission denied / unsupported / desktop), that coaxing is just
    // unescapable punishment — so skip it and leave the icy GRIP loss alone
    // (snow stays slippery-but-drivable on L/R + tap).  When tilt IS attached,
    // default mode is already on tilt here so this still applies under it.
    const _tiltCoax = !!this._tiltAttached;
    if (_snowRamp > 0 && _tiltCoax) {
    ...
    const releaseScale = effectiveSteerDir === 0
                         ...
                         : 1;
    // Base settle rate 8 (per the user's pseudocode) → ~63 % toward target
    // in 0.12 s at full grip.  Stability boosts the settle rate (a planted
    // truck snaps to its target lateral faster even though it turns slowly
  ...
  _updateViceDrift(rawDt) {
    ...
    // The pickup is a slow object ON the road: it has its OWN absolute top
    // speed of 20 mph and crawls downroad at min(yourSpeed, 20).  So if you're
    // doing 100 it only does 20 (you close at ~80); if you stop, it stops.
    // NOT relative-to-player — the sprite never physically exceeds 20 mph.
    const capUnits = MAX_SPEED * 20 / 120;                  // 20 mph absolute cap
    const advance  = Math.min(this.player.speed, capUnits) * rawDt;
    if (advance <= 0) return;                               // player stopped → pickup holds still
    const startSeg = Math.floor(this.player.position / SEG_LENGTH);
    const WINDOW   = Math.min(400, nSeg - 1);               // just past draw distance; never wrap the array
    const moves    = [];
    for (let n = 0; n <= WINDOW; n++) {
      const fromIdx = (startSeg + n) % nSeg;
      ...
      for (const sp of seg.sprites) {
        ...
        if (sp._driftAccum >= SEG_LENGTH) {
          ...
          sp._driftAccum -= hop * SEG_LENGTH;
          moves.push({ sp, fromIdx, toIdx: (fromIdx + hop) % nSeg });
        }
      }
    }
    // Apply segment moves AFTER the walk so we never mutate an array mid-iterate.
    for (const { sp, fromIdx, toIdx } of moves) {
      const from = segs[fromIdx].sprites;
      const i = from.indexOf(sp);
      if (i >= 0) from.splice(i, 1);
      (segs[toIdx].sprites ??= []).push(sp);
    }
  }

  /** Radar detector — when owned, beep + flash a dashboard light through the
   *  last half-mile before any speed trap, escalating (faster blips, brighter
   *  light) as the trap nears.  Always pings (no speed gate — everyone's
   *  speeding).  Trap mile positions come from the route (the same data the
   *  friend's far warning uses); custom "no police" disarms it. */
  _updateRadar(rawDt) {
    const gfx = this._radarGfx, label = this._radarLabel;
    if (!gfx) return;
    const hide = () => {
      if (gfx.visible) { gfx.clear(); gfx.setVisible(false); }
    ...
    const WINDOW  = 0.5;   // alert through the last half-mile
    const mileNow = (this.player.position / (ROUTE_SEGS * SEG_LENGTH)) * TOTAL_ROUTE_MILES;
    let nearest = Infinity;
    for (const tm of trapMiles) {
      const d = tm - mileNow;                 // only traps AHEAD
      if (d > 0 && d < nearest) nearest = d;
    }
    if (nearest > WINDOW) { hide(); return; }

    const prox = Math.max(0, Math.min(1, 1 - nearest / WINDOW));   // 0 far → 1 at trap
    // Escalating blip cadence: ~0.55 s at the far edge → ~0.10 s at the trap.
    const interval = 0.55 - prox * 0.45;
    this._radarBeepTimer += rawDt;
    if (this._radarBeepTimer >= interval) {
      this._radarBeepTimer = 0;
      this._radarBlinkOn   = !this._radarBlinkOn;
      this.audio?.playRadarBeep?.(prox);
    }
    // Dashboard light — a red dot blinking in time with the blips, growing
    // and brightening as the trap nears; "SPEED TRAP" label beneath it.
    const cx = 52, cy = 100, r = 6 + prox * 5;
    const on = this._radarBlinkOn;
    gfx.setVisible(true);
    gfx.clear();
    gfx.fillStyle(0xFF2222, on ? 0.95 : 0.22);
    gfx.fillCircle(cx, cy, r);
    gfx.lineStyle(2, 0xFFFFFF, on ? 0.9 : 0.3);
    gfx.strokeCircle(cx, cy, r);
    if (label) { label.setVisible(true).setAlpha(on ? 1 : 0.5); }
  }

  // ─── Rear-view mirror geometry (build + touch-hold zoom) ─────────────
  /** Derive every mirror dimension from an outer w/h.  Single source of
   *  truth shared by the initial build and _setMirrorZoom so a zoomed
   *  mirror is laid out byte-identically to the resting one. */
  _mirrorGeom(mw, mh) {
    // Custom-layout offset (Customize Controls) shifts the whole mirror; mx/my
    // cascade into the glass/road geometry below so the interior follows.
    const mx = SCREEN_W / 2 - mw / 2 + (this._mirrorUserDx || 0);   // centred + custom dx
    const my = 2 + (this._mirrorUserDy || 0);                       // top-anchored + custom dy
    const z = this._mirrorBaseW ? mw / this._mirrorBaseW : 1;
    const inset = 4 * z;
    const glassX = mx + inset, glassY = my + inset;
    const glassW = mw - inset * 2, glassH = mh - inset * 2;
    const horizonY  = glassY + glassH * 0.42;
    const roadCx    = glassX + glassW / 2;
    const roadBotY  = glassY + glassH - 2 * z;
  ...
  /** Repaint the static mirror frame from the current _mirrorBounds.
   *  Corner radius scales with zoom so the housing stays proportional. */
  _drawMirrorFrame() {
    const f = this.hudMirrorBg, b = this._mirrorBounds;
    if (!f || !b) return;
    const r = 9 * (b.w / this._mirrorBaseW);
    f.clear();
    f.fillStyle(0x1A1A1A, 0.95);
    f.fillRoundedRect(b.x, b.y, b.w, b.h, r);
    f.lineStyle(2, 0x666666, 1);
    f.strokeRoundedRect(b.x + 0.5, b.y + 0.5, b.w - 1, b.h - 1, r);
  }

  /** Re-fill the geometry-mask shape to the current glass rect.  Re-fills
   *  the SAME graphics object (never recreates the mask) so every pooled
   *  mirror sprite keeps its existing _mirrorMask reference. */
  _refillMirrorMask() {
    const ms = this._mirrorMaskShape, b = this._mirrorBounds;
    if (!ms || !b) return;
    ms.clear();
    ms.fillStyle(0xFFFFFF, 1);
    ms.fillRect(b.glassX, b.glassY, b.glassW, b.glassH);
  }

  /** Set the mirror zoom factor (1 = resting, _mirrorHoldZoom = held).  Recomputes
   *  bounds from the base size, redraws the frame, and re-fills the mask;
   *  the glass interior + sprite pools follow automatically because the
   *  per-frame render reads _mirrorBounds. */
  _setMirrorZoom(z) {
    if (!this._mirrorBaseW || !this.hudMirrorBg) return;
    if (this._mirrorZoom === z) return;
    this._mirrorZoom   = z;
    const us = this._mirrorUserScale || 1;   // custom-layout scale rides with the hold-zoom
    this._mirrorBounds = this._mirrorGeom(this._mirrorBaseW * us * z, this._mirrorBaseH * us * z);
    this._drawMirrorFrame();
    this._refillMirrorMask();
  }
  ...
  _spawnTraffic() {
    const p         = this.player;
    // Ambient cop traffic ramps with wanted level but stays modest: ~5% per
    // star (1★≈5%, 5★≈25%), capped at 25% so high stars don't turn the road
    // into a parade of cruisers.  (Active pursuit cops + roadside speed traps
    // layer on top via their own systems — this is just background cop cars.)
    const isCop     = this.cops.stars >= 1 && Math.random() < Math.min(0.25, this.cops.stars * 0.05);
    ...
    //      (Semi share bumped ~15% in every band 2026-06-21, taken from car.)
    //      17–52          : car ~89.9 / white_truck 6 / work_truck 3 / semi ~1.15.
    //                        Light truck traffic on the I-90 climb.
    //      53–69          : car ~81.4 / white_truck 8 / work_truck 6 / semi ~4.6.
    //                        Coming off the pass, freight ramps up.
    //      70–136         : car ~68.2 / white_truck 10 / work_truck 8 / semi ~13.8.
    //                        Easton → Vantage backbone — real semi country.
    //      137+ (E. WA)   : car ~46.7 / semi ~25.3 / white_truck 10 / work_truck 9 / tractor 9.
    ...
    if (!isCop) {
      ...
      if (mileNow < 17) {
        ...
        if      (r < 0.8985) vClass = 'car';
        else if (r < 0.9585) vClass = 'white_truck';
        else if (r < 0.9885) vClass = 'work_truck';
        else                 vClass = 'semi';
      } else if (mileNow < 70) {
        if      (r < 0.814) vClass = 'car';
        else if (r < 0.894) vClass = 'white_truck';
        else if (r < 0.954) vClass = 'work_truck';
        else                vClass = 'semi';
      } else if (mileNow < 137) {
        if      (r < 0.682) vClass = 'car';
        else if (r < 0.82)  vClass = 'semi';
        else if (r < 0.92)  vClass = 'white_truck';
        else                vClass = 'work_truck';
      } else {
        if      (r < 0.467) vClass = 'car';
    ...
    // ~50% fewer SAME-DIRECTION NPCs (restored per user).  Abort half of the
    // same-direction CIVILIAN spawns — oncoming, COPS, and rare tractors are
    // unaffected (the cull never touched police).  The spawn timer retries
    // next tick, so steady-state same-dir civilian count halves.
    if (!oncoming && !isCop && vClass !== 'tractor' && Math.random() < 0.5) return;
    ...
    //                  EASY → 80 ± 5 same-dir / 45 ± 5 oncoming (see below)
    //   semi         : 70 ± 10              / 60 ± 8
    ...
    const _easyMode = Difficulty.mode?.() === 'easy';
    let baseMph, spreadMph;
    switch (vClass) {
      // Oncoming trucks: Easy 35 / Normal-Hard 50 (per Brendan).  Same-direction
      // semi 70, white_truck 75.  work_truck is its own thing: same-dir 60,
      // oncoming a flat 45 (per Brendan).
      case 'semi':        baseMph = oncoming ? (_easyMode ? 35 : 50) : 70; spreadMph = oncoming ? 8  : 10; break;
      case 'white_truck': baseMph = oncoming ? (_easyMode ? 35 : 50) : 75; spreadMph = oncoming ? 12 : 15; break;
      case 'work_truck':  baseMph = oncoming ? 45 : 60;                    spreadMph = 5;                  break;
      case 'tractor':     baseMph = 30;                 spreadMph = 3;                  break;
      case 'car':
      default:
        if (_easyMode) {
          // Easy — tighter, more avoidable car traffic: same-direction cars
          // hold a consistent 75-85 (low relative-speed overtakes) and
          // oncoming cars drop to 40 mph (gentler head-on closing speed).
          baseMph = oncoming ? 40 : 80; spreadMph = 5;
        } else {
          // Normal/Hard — oncoming cars base 55 mph (per Brendan, to soften the
          // head-on closing speed), tight ±10 mph spread (≈45–65).
          baseMph = oncoming ? 55 : 80; spreadMph = oncoming ? 10 : 25;
        }
        break;
    }
    ...
    // Fog — most cars ease ~20 mph off in the low visibility (~1 in 7 still
    // barrels through).  Floored at 10 mph so nobody crawls to a stop.
    if (!isCop && Weather.isFog(mileNow) && Math.random() < 0.85) {
      trafficSpeed = Math.max(MAX_SPEED * 10 / 120, trafficSpeed - MAX_SPEED * 20 / 120);
    }
    ...
    // bump to 1.45 so they read as imposing/over-lane-wide (makes squeezing
    // between paired semis genuinely tight).
    // Tractor uses 1.10 (wider than a car, narrower than a semi).
    let visualScale = 1;
    if (vClass === 'semi')    visualScale = 1.45;
  ...
  _checkCollisions() {
    ...
    // CRITICAL: project pickups from the SAME camera basis the renderer uses
    // (`_renderCamPos` = player.position + CAM.eyeForwardZ).  In cockpit view
    // eyeForwardZ is 4500 (≈ 22 segments), so scanning from the raw
    // player.position put the collision check ~22 segments BEHIND where the
    // pickup was actually drawn — the sprite visually touched the bumper but
    // collision still thought it was far ahead, so it never collected (worst
    // on the centerline / at speed).  Chase view has eyeForwardZ 0, so this
    // is a no-op there (matches the old behavior).  `segIdx` (raw) is left
    // alone for the scenery-collision loop below, which uses its own path.
    const camSegIdx = Math.floor(this._renderCamPos() / SEG_LENGTH) % this.road.segments.length;
    for (let di = 0; di <= 16; di++) {
      const idx = (camSegIdx + di) % this.road.segments.length;
    ...
    for (let i = this.cops.cops.length - 1; i >= 0; i--) {
      ...
      // PIT branch (armed side-swipe → PIT strike).  A stale
  ...
  /** NPC ↔ NPC and cop ↔ NPC crashes — vehicles no longer phase through each
   *  other (cops used to drive straight through traffic).  Same spirit as the
   *  player's crash rules: a hard enough hit WRECKS the car that gets rammed
   *  (the front car in a rear-end; BOTH in a head-on), 18-wheelers are
   *  immovable, and police have HP (10; SWAT 20) so they only wreck once it's
   *  gone.  A conservative closing-speed gate means normal follow-distance
   *  jostling never triggers a crash — only real differential / lane-merge /
   *  oncoming impacts do.  Runs right after _checkCollisions() (the player). */
  _updateVehicleCrashes() {
    const now     = this.time?.now ?? 0;
    const traffic = this.traffic ?? [];
    const cops    = this.cops?.cops ?? [];
    const list = [];
    for (const c of traffic) if (c.alive && !c.crashed) list.push({ v: c, cop: false });
    for (const c of cops)    if (c.alive && !c.parked)  list.push({ v: c, cop: true });
    if (list.length < 2) return;

    const explodeAt = (v) => {
      const relZ = v.position - this.player.position;
      if (relZ <= 0) return;                          // behind the camera → off-screen, skip FX
      const proj = this.road.getVehicleProjection(relZ, v.laneOffset);
      if (proj) this._spawnExplosion(proj.sx, proj.sy, proj.sw);
    };
    // victim=true → this car got rammed (wrecks); victim=false → aggressor
    // (the car that did the ramming; survives, just scrubs speed + nudges).
    const applyHit = (v, isCop, victim, sev, headOn, push) => {
      if (v.vClass === 'semi') return;                // 18-wheeler is immovable (matches player rules)
      if (isCop) {
        const dmg = victim ? (headOn ? 6 + sev * 8 : 2 + sev * 3) : (1 + sev * 1.5);
        v.hp = (v.hp ?? (v.colorSet === 'swat' ? 20 : 10)) - dmg;
        if (v.hp <= 0) { v.alive = false; explodeAt(v); }
        else {
          v.speed *= clamp(0.80 - sev * 0.18, 0.5, 0.85);
          v.laneOffset = clamp((v.laneOffset ?? 0) + push * 0.04, -1.2, 1.2);
        }
      } else if (victim) {
        // Wreck the NPC exactly like the player crashing it (spin-out + smoke).
        v.alive = false; v.crashed = true; v.crashTimer = 1.6;
        v.crashVx = push * (0.9 + sev * 0.8) + (Math.random() - 0.5) * 0.4;
        v.crashAng = 0;
        v.crashSpin = (Math.random() < 0.5 ? -1 : 1) * (2.5 + sev * 3.5 + Math.random() * 2);
        v.crashSmokeT = 0;
        v.speed *= clamp(0.60 - sev * 0.20, 0.28, 0.55);
        explodeAt(v);
      } else {
        v.speed *= clamp(0.82 - sev * 0.15, 0.55, 0.9);
        v.laneOffset = clamp((v.laneOffset ?? 0) + push * 0.04, -1.2, 1.2);
      }
    };

    const CLOSING_MIN_MPH = 12;                       // below this = gentle; follow-distance owns it
    for (let i = 0; i < list.length; i++) {
      const A = list[i], a = A.v;
      if (!a.alive || a.crashed || now < (a._vehHitUntil ?? 0)) continue;
      for (let j = i + 1; j < list.length; j++) {
        const B = list[j], b = B.v;
        if (!b.alive || b.crashed || now < (b._vehHitUntil ?? 0)) continue;
        if (Math.abs(a.position - b.position) >= CAR_LEN_Z) continue;
        if (Math.abs((a.laneOffset ?? 0) - (b.laneOffset ?? 0)) >= CAR_WIDTH_LANES * 2) continue;
        const av = a.speed ?? 0, bv = b.speed ?? 0;
        const sameDir    = (av >= 0) === (bv >= 0);
        const closing    = sameDir ? Math.abs(av - bv) : (Math.abs(av) + Math.abs(bv));
        const closingMph = closing / MAX_SPEED * 120;
        if (closingMph < CLOSING_MIN_MPH) continue;
        const sev    = Math.min(1, closingMph / 70);
        const headOn = !sameDir;
        const aPush  = ((a.laneOffset ?? 0) >= (b.laneOffset ?? 0)) ? 1 : -1;
        let aVictim, bVictim;
        if (headOn) { aVictim = true; bVictim = true; }
        else {
          const forward = av >= 0;                    // front car (gets rear-ended) is the victim
          const aFront  = forward ? (a.position > b.position) : (a.position < b.position);
          aVictim = aFront; bVictim = !aFront;
        }
        applyHit(a, A.cop, aVictim, sev, headOn,  aPush);
        applyHit(b, B.cop, bVictim, sev, headOn, -aPush);
        a._vehHitUntil = now + 400;                   // per-car cooldown so one crash isn't applied 60×/s
        b._vehHitUntil = now + 400;
        if (!a.alive || a.crashed) break;             // a is gone → stop pairing it this frame
      }
    }
    // Remove wrecked cops (NPC wrecks despawn via their own crash animation).
  ...
  _buildCustomModeModal({ onConfirm, onClose, initialLevels = null } = {}) {
    ...
    let damageBtn;
    // Description mirrors the toggle: ON = car takes damage, OFF = no damage.
    const damageDescText = () => noNpcDamage ? 'PLAYER CAR TAKES NO DAMAGE' : 'PLAYER CAR TAKES DAMAGE';
    damageBtn = makeToggleButton(rightX + 66, damageY - 2, 80, 27, 'ON', 0xFF39AF, true, () => {
      ...
      damageDesc.setText(damageDescText());
    });
    const damageDesc = this.add.text(rightX + 156, damageY + 11, damageDescText(), {
      fontSize: '12px', fontFamily: IMPACT, color: '#A8C7E9',
    }).setOrigin(0, 0.5).setDepth(D + 5);
    add(damageDesc);
    ...
    driveOptions.forEach((opt, i) => {
      // Four buttons now share the same row width the old three occupied
      // (rightX+103 .. +355): 60-wide cells on a 64 px step.
      const btn = makeToggleButton(rightX + 103 + i * 64, driveY, 60, 28,
  ...
  _applyDamage(amount, source) {
    ...
    if (isCollision) {
      ...
      const isContinuousScrape =
        source.startsWith('offroad')   // offroad_bleed, offroad_left
  ...
  _onVehicleCollision(car, _idx, hit) {
    ...
    if (type === 'rear-end') {
      ...
      if (isSemi) {
        // An 18-wheeler is immovable — rear-ending one (or meeting it head-on)
        // does NOT flip, spin, or destroy it.  The player bounces off, takes
        // the heavy hit, and scrubs down; the semi keeps rolling.  (Matches the
        // sideswipe / corner semi branches below — only the player reacts.)
        this._spawnExplosion(sx, sy, sw);
        ...
        this._showPopup(isHeadOn ? 'HEAD-ON WITH A SEMI!' : 'REAR-ENDED A SEMI!', '#FF2222');
  ...
  _onCopCollision(cop, idx, hit) {
    ...
    if (kind === 'rear' && type !== 'side-swipe') {
      ...
      // Cops have HP (10; SWAT 20).  A rear ram chips 1 — so ~10 rams
      // wreck a fresh cruiser, same as before — but crashes into NPC
      // traffic chip the SAME pool (see _updateVehicleCrashes), so a
      // cruiser that's been smashing through cars wrecks in fewer rams.
      cop.hp = (cop.hp ?? (cop.colorSet === 'swat' ? 20 : 10)) - 1;
      if (cop.hp <= 0) {
        this._spawnExplosion(sx, sy, sw);
        cop.alive = false;
        this.cops.cops.splice(idx, 1);
    ...
    if (type === 'side-swipe' && cop._pitArmed) {
      ...
      const pits     = this.cops.registerPit();
      const pitMax   = Difficulty.arrest().pit;
      const left     = pitMax - pits;
      this._showPopup(
        left > 0 ? `PIT MANEUVER! ${pits}/${pitMax}\n${left} more = JAIL` : 'BUSTED!',
        '#FF2222',
      );
      cop.alive = false;
      this.cops.cops.splice(idx, 1);
  ...
  _hitchhikerPickup() {
    ...
    if (r < 0.28) {
      this.cops.addF12Token('paint_bomb');
  ...
  _applyPedalHandedness() {
    // Pedals are FIXED at the bottom-centre (BRAKE left of the mileage,
    // ACCEL right of the location) and no longer mirror with handedness —
    // just re-assert the layout (also re-anchors the wiper).
    this._layoutPedalsToText();
  }

  /** Place the BRAKE + ACCEL pedals on the bottom row at their fixed X's
   *  (BRAKE left of the mileage, ACCEL right of the location — see the
   *  _buildHUD pedal block), publish the two steer-exclusion zones, and
   *  re-anchor the weather/wiper button beside the brake. */
  _layoutPedalsToText() {
    if (!this._gasBtn || !this._brakeBtn) return;
    if (this._brakePedalX == null || this._accelPedalX == null) return;  // pre-build guard (no NaN)
    const PW = this._pedalDim?.w ?? 70;
    const PH = this._pedalDim?.h ?? 50;
    ...
    this._pedalHitZones = [
      { x: bx - PW / 2, y: rowY - PH, w: PW, h: PH },
      { x: ax - PW / 2, y: rowY - PH, w: PW, h: PH },
    ];
  ...
  _togglePause() {
    ...
    if (this._paused) {
      // (Dim film removed per user request — PAUSED text has a heavy outline and
      // the buttons carry their own backgrounds, so the frozen world shows clear.)
    ...
    // ...and CLEAR the transient touch-steer flags.  On a phone the menu
    // (Messages, Stats, etc.) is entered/exited by ROTATING, not by a tap,
    // so the pointerup that normally resets these never fires — without this
  ...
  /** Collect roadside cop ENCOUNTER sprites (parked speed traps + ambient
   *  driving cops) near the player so a weapon can target them like any
   *  cruiser.  Each entry carries a world position (player.position + relZ,
   *  so the weapon's `pos - playerPos` range math just works), its lateral
   *  offset, and its home seg.sprites array (so a hit splices it off the
   *  road).  Scans a window AROUND the player — behind AND ahead — since
   *  weapons fire both directions. */
  _collectEncounterCops() {
    const segs = this.road?.segments;
    if (!segs?.length) return [];
    const nSeg = segs.length;
    const segIdx = Math.floor(this.player.position / SEG_LENGTH);
    const out = [];
    for (let di = -60; di <= 200; di++) {
      const idx = ((segIdx + di) % nSeg + nSeg) % nSeg;
      const seg = segs[idx];
      if (!seg?.sprites?.length) continue;
      for (const sp of seg.sprites) {
        if (!sp.copEncounter || sp.collected) continue;
        out.push({
          sp, segSprites: seg.sprites,
          position: this.player.position + di * SEG_LENGTH,
          offset: sp.offset ?? 0,
          colorSet: 'police',
        });
      }
  ...
  // ─── Render ───────────────────────────────────────────────────────────
  _renderFrame() {
    // Keep the decoupled-width centering locked in every frame.  Applying it
    // only at create()/resize raced the boot sequence — when GameScene.create
    // ran before main.js's first applyOrientation, HUD_OFFSET_X was still 0, so
    // the world camera was left un-scrolled (car drifted left of centre).  Re-
    // asserting here is cheap (3 camera ops) and immune to that ordering.
    this._applyDecoupledCameras?.();
    const progress = this.player.position / (ROUTE_SEGS * SEG_LENGTH);
  ...
  _renderHeadlights() {
    ...
    const npcFogDensityForLights = Weather.fogParams?.(mile)?.density ?? 0;
    if (TimeOfDay.headlightsOn?.(mile) && npcFogDensityForLights <= 0.05 && this.traffic?.length) {
   ...
   *  done (or if there's nothing to wait for). */
  _ensurePlayerArtReady() {
    if (!this._playerArtPending) return;                   // not waiting (or already applied)
    const key = this._playerArtKey;
    if (!key || !this.playerSprite || !this.textures.exists(key)) return;  // still loading — wait
    this._playerArtPending = false;
    this.playerSprite.setTexture(key).setVisible(true);
    this._applyPlayerSpriteDisplaySize();
  }
  ...
  _renderVehicles() {
    ...
    const outlinePool = this._carOutlinePool;
    // OUTLINE rim tunables (see pool init).  A bright rim-light edge pops the
    // car off the grey road WITHOUT darkening it; flip COLOR to a dark value
    // (e.g. 0x0A0A0A) for an ink edge instead.  PX sets the rim thickness as
    // a fraction of the car's on-screen width.
    const CAR_OUTLINE_COLOR = 0xF2F5FF;   // bright rim-light edge
    const CAR_OUTLINE_ALPHA = 0.85;
    const CAR_OUTLINE_PX    = 0.045;      // rim ≈ this × car width (clamped 0.8–3 px)
    ...
    // Depth-fog (Phase 1): fade distant cars/cops/wrecks toward the fog
    ...
    const place = (relZ, laneOffset, color, scaleHint, rotation, texKey, noGhost, maxW, lightsKind = 'tail', vehicleKind = '') => {
      ...
      if (shadowG) {
        ...
        shadowG.fillStyle(0x000000, 0.32 * _shadowFa);   // fades with the car + softens in fog
        shadowG.fillEllipse(proj.sx, shY, shW, shH);
        if (ghostOffsetBase > 0) {
          const ghostOffset = ghostOffsetBase * Math.min(1, (proj.sw ?? 0) / 200);
          shadowG.fillStyle(0x000000, 0.32 * ghostAlpha * _shadowFa);
          shadowG.fillEllipse(proj.sx + ghostOffset, shY, shW, shH);
        }
      }

      // ── Fog light-glow — the car's actual lights diffused through fog:
      // broad soft BLOOM first, then only a tiny muted core.  Fog should read
      // as glow/mist, not crisp clear-night shine.  FAINT when far,
      // brightening smoothly as the car
      // approaches.  Red tail same-direction, warm headlights oncoming,
      // blue/red bar for cops.  Skipped for wrecks (lightsKind = null).
      if (lightsKind && this._fogGlowGfx && _fogP.density > 0.05) {
        const d     = _fogP.density;
        const prox  = Math.max(0, Math.min(1, 1 - relZ / 50000));  // 0 far → 1 near
        // appearF: fade IN out of the far haze (no pop-in at max distance).
        // nearF:   keep the haze alive until the car is right beside/behind
        //          the player, then fade it to just the bulbs (was: faded out
        //          ~40 yds early via (1-prox)).
        // fade: exponential fog distance-attenuation (1 near → 0 far) so a
        //       light hundreds of feet back dims to a faint smudge and then
        //       vanishes — no more far lights as bright as the player's.
        //       sqrt() lets a light cut through slightly more fog than the
        ...
        const coreA = d * (0.05 + 0.38 * prox) * appearF * fade;   // muted bulb: glow, not shine
        const hazeA = d * 0.42 * appearF * nearF * fade;           // stronger mist bloom
        // Headlights have to cut through Issaquah's thick mile 15-23 fog
        // farther than the dark car body does, otherwise oncoming traffic
        // reads unlit until it is nearly on top of the player.
        const headCarry = d * appearF * Math.exp(-relZ / 22000);
        const headCoreA = Math.max(coreA, d * (0.025 + 0.22 * prox) * headCarry);
        const headHazeA = Math.max(hazeA, d * 0.26 * nearF * headCarry);
        const headSharpA = Math.min(0.95, d * (0.28 + 0.50 * prox) * Math.max(0, 1 - relZ / 6500));
        const hazeR = baseR * (2.4 + (1 - prox) * 2.8);            // bigger soft fog aura
        const g     = this._fogGlowGfx;
        // Two lamps that start MERGED at distance (one soft oval) and pull
        // apart as the car nears; even close-up the core stays subdued so the
        // light feels scattered by fog rather than a hard LED dot.
        const pair = (color, dxFull, opts = {}) => {
          const dx = dxFull * (0.12 + 0.88 * prox);
          const y = opts.y ?? tailY;
          const hA = (opts.hazeA ?? hazeA) * (opts.hazeMul ?? 1);
          const cA = (opts.coreA ?? coreA) * (opts.coreMul ?? 1);
          const hR = hazeR * (opts.hazeScale ?? 1);
          const bR = baseR * (opts.coreScale ?? 0.28);
          if (hA > 0.01) {
            g.fillStyle(color, hA * 0.34); g.fillEllipse(proj.sx, y, dx * 2 + hR * 2.6, hR * 1.55);
            g.fillStyle(color, hA * 0.62); g.fillEllipse(proj.sx, y, dx * 2 + hR * 1.7, hR * 1.05);
            g.fillStyle(color, hA * 0.48); g.fillEllipse(proj.sx, y, dx * 2 + hR * 0.9, hR * 0.62);
          }
          g.fillStyle(color, cA);
          g.fillCircle(proj.sx - dx, y, bR);
          g.fillCircle(proj.sx + dx, y, bR);
          if ((opts.sharpA ?? 0) > 0.01) {
            g.fillStyle(opts.sharpColor ?? color, opts.sharpA);
            g.fillCircle(proj.sx - dx, y, bR * 0.56);
            g.fillCircle(proj.sx + dx, y, bR * 0.56);
          }
        };
        if (lightsKind === 'cop') {
        ...
        } else if (lightsKind === 'head') {
          // Oncoming headlights should catch the eye in fog, but stay as a
          // soft lamp glow instead of the clear-night beam/cone treatment.
          // As the car reaches the player, haze fades but a small sharp bulb
          // remains so the lights do not appear to switch off.
          pair(0xFFE2A0, headDxFull, { y: headY, hazeA: headHazeA, coreA: headCoreA, hazeMul: 2.50, coreMul: 2.16, hazeScale: 1.08, coreScale: 0.27, sharpA: headSharpA, sharpColor: 0xFFF4C8 });
        } else {                            // 'tail'
          // Tail bulbs need to read through fog as they approach, but the
          // haze should still fall away near the player via nearF above.
          pair(0xFF2A14, tailDxFull, { y: tailY, hazeMul: 1.00, coreMul: 1.32, hazeScale: 0.92, coreScale: 0.30 });
        }
      }
      ...
      // ── OUTLINE rim — a larger, tinted copy of the same sprite drawn just
      // behind the car so a thin edge peeks around it.  Centred on the car's
      // MIDDLE (origin 0.5,0.5) so the rim is even; thickness scales with the
      // on-screen size so distant cars don't get a chunky edge.
      const ol = outlinePool?.[slotIdx];
      if (ol) {
        ...
        ol.setPosition(proj.sx, proj.sy - targetH / 2)
          ...
          .setAlpha(CAR_OUTLINE_ALPHA * _fa)
          .setVisible(true);
      }

      // Double-vision ghost copy — shifted laterally and alpha'd.
      // Scale by perspective so far sprites don't get a huge displacement.
      // `noGhost` opts a sprite out (e.g. the parked PULL-OVER cruiser, which
      // must stay solid/legible even while the player is seeing double).
      if (ghostOffsetBase > 0 && !noGhost && ghostPool && ghostUsed < ghostPool.length) {
        const ghostOffset = ghostOffsetBase * Math.min(1, (proj.sw ?? 0) / 200);
    ...
    for (const t of this.traffic) {
      ...
      if (t.crashed) {
        place(relZ, t.laneOffset, isImg ? 0xAA8866 : 0x664422, vs, t.crashAng ?? 0, texKey, undefined, undefined, null, t.vClass ?? t.colorSet);
      } else if (t.alive) {
        const inFogLights = (_fogP?.density ?? 0) > 0.05;
        ...
        if (!inFogLights && (t.speed ?? 0) >= 0 && TimeOfDay.headlightsOn?.(
    ...
    if (shadowG && this.playerSprite?.visible !== false && !this._sinkState) {
      ...
      shadowG.fillStyle(0x000000, 0.40 * (1 - (_fogP?.density ?? 0)));   // no sun through fog → player shadow gone at peak fog
      shadowG.fillEllipse(0, 0, shW, shH);
      shadowG.restore();
    }

    // Player car tail lights — lit red in the fog (you run lights too).  Two
    // oval lamps (bright core + soft halo) at the rear; the player's always
    // close, so they read at full brightness scaled by fog density.
    if (this._fogGlowGfx && (_fogP?.density ?? 0) > 0.05
      ...
      const glow = (cx) => {
        g.fillStyle(0xFF1A00, a * 0.20); g.fillCircle(cx, py, r * 2.1);   // broad soft fog bloom (haze veil over the lamp)
    ...
    if (outlinePool) {
      for (let i = used; i < outlinePool.length; i++) outlinePool[i].setVisible(false);
    }
    if (ghostPool) {
  ...
  _renderUtilityLines() {
    ...
    const camPos = this._renderCamPos();
    const startSeg = Math.floor(camPos / SEG_LENGTH);
    // 2026-06-06 rewrite: a power line is NOT a fence rail.  The previous
    // pass sampled the road densely (every WIRE_STEP segments, fence-rail
    // cadence) so the wire "followed the road surface exactly like the
    // fence rail" — that was the bug: it bent the wire DOWN toward the road
    // between poles and dove to the ground near the camera.  A real wire
    // ignores the terrain between poles; it spans pole-to-pole.  So wires
    // and pole sprites now share ONE pass at real pole SPACING (≈200 ft):
    // each pole is an anchor, and a straight line joins consecutive anchors.
    const SPACING = 61;

    const firstPoleSeg = Math.ceil((startSeg + 1) / SPACING) * SPACING;
    const lastPoleSeg  = Math.floor((startSeg + DRAW_DIST - 1) / SPACING) * SPACING;
    let usedPoles  = 0;
    let prev       = null;   // nearest-yet pole anchor (the wire's near end)
    let secondPrev = null;   // the pole one farther (gives the near-end slope)

    for (let absoluteSeg = lastPoleSeg; absoluteSeg >= firstPoleSeg; absoluteSeg -= SPACING) {
      const wrappedSeg = ((absoluteSeg % segs.length) + segs.length) % segs.length;
      const seg = segs[wrappedSeg];
      const side = seg?.utilityLineSide ?? 0;
      // Gap (no line here / bridge / tunnel / water / rural fence): break the
      // run so a wire never spans across it.
      ...
      if (!p || p.sw < 1.1) { prev = null; secondPrev = null; continue; }

      // Wire height comes ONLY from the pole's own crossarm (this same
      // projection drives the pole sprite below) — never from a road sample.
      // The upper clamp is well past screen height so a nearing pole keeps
      // GROWING (top off the top, base off the bottom) like the rest of the
      // roadside scenery, instead of freezing at 190 px and sliding down the
      // screen as its base scrolls past the bottom.  poleW tracks poleH so
      // the whole sprite scales uniformly.
      const poleH  = clamp(p.sw * 3.35, 4, SCREEN_H * 4);
      const poleW  = poleH * 0.5;
      const wireA  = p.sy - poleH * 0.94;
      const wireB  = p.sy - poleH * 0.90;
      const anchor = { x: p.sx, wireA, wireB, poleH, side };

      // A side switch breaks the run (don't span across the road).
      if (prev && prev.side !== side) { prev = null; secondPrev = null; }

      // ── Wire span: straight line from the previous pole's crossarm to
      //    this one.  Every visible pole is an anchor — including the
      //    nearest — so the wire stays attached to the pole it's nearest to.
      if (prev) {
        g.lineStyle(clamp(poleH * 0.5 * 0.025, 0.7, 1.6), 0x26282A, 0.85);
        g.beginPath();
        g.moveTo(prev.x, prev.wireA); g.lineTo(anchor.x, wireA);
        g.moveTo(prev.x, prev.wireB); g.lineTo(anchor.x, wireB);
        g.strokePath();
      }

      // ── Pole sprite at this anchor (poles still render right up to the
      //    camera even when their wire span is skipped). ────────────────
      ...
      // This pole becomes the wire's near end (track the prior one for slope).
      secondPrev = prev;
      prev = anchor;
    }

    // ── Near-end continuation ─────────────────────────────────────────
    // Continue the wire from the nearest pole's crossarm to the screen edge
    // along the SAME perspective slope as the last real span (secondPrev →
    // prev) — pole tops rise toward the camera, so the wire keeps climbing
    // off the top instead of flat-lining.  Floored at the nearest crossarm:
    // if that slope would point DOWN (end-of-run / downhill), it holds flat
    // (constant Y) instead — the wire never drops below the pole it leaves.
    if (prev) {
      const dx = secondPrev ? (prev.x - secondPrev.x) : 0;
      const heading = (secondPrev && Math.abs(dx) > 1);
      const edgeX = heading
        ? (dx >= 0 ? (SCREEN_W + 40) : -40)
        : (prev.x > SCREEN_W * 0.5 ? (SCREEN_W + 40) : -40);
      let eyA = prev.wireA, eyB = prev.wireB;
      if (heading) {
        const t = clamp((edgeX - prev.x) / dx, 0, 8);   // span-lengths past `prev`
        eyA = Math.min(prev.wireA, prev.wireA + (prev.wireA - secondPrev.wireA) * t);
        eyB = Math.min(prev.wireB, prev.wireB + (prev.wireB - secondPrev.wireB) * t);
      }
      g.lineStyle(clamp(prev.poleH * 0.5 * 0.025, 0.7, 1.6), 0x26282A, 0.85);
      g.beginPath();
      g.moveTo(prev.x, prev.wireA); g.lineTo(edgeX, eyA);
      g.moveTo(prev.x, prev.wireB); g.lineTo(edgeX, eyB);
      g.strokePath();
    }
  ...
  _renderSceneSprites() {
    ...
    let used = 0;
    let usedPlanes = 0;
    // Alcohol double-vision for scenery — capped ghost pass.  Only the near,
    // large sprites (proj.sw ≥ threshold) get a ghost, prioritised because the
    // loop runs far→near so the pool isn't burned on tiny distant scenery.
    const _dvScn         = this._debugOn ? 0 : (this.effects?.doubleVision ?? 0);
    const _scnGhostOn    = _dvScn > 0.01;
    const _scnGhostBase  = _scnGhostOn ? _dvScn * 38 : 0;
    const _scnGhostAlpha = _scnGhostOn ? _dvScn * 0.62 : 0;
    const _scnGhostPool  = this._sceneGhostPool ?? [];
    const _SCN_GHOST_MIN_SW = 55;   // only near scenery ghosts readably
    let _scnGhostUsed = 0;
    ...
    for (let n = 0; n <= 1000 && used < pool.length; n++) {
      ...
      for (const sp of seg.sprites) {
        ...
        s.setPosition(_renderX, spriteBaseY)
          .setDisplaySize(targetW, targetH)
          .setDepth(depth)
          .setAlpha(fadeAlpha * Weather.fogFade(relZ, _fogPScn))
        ...
        // Double-vision ghost — mirror this (near, large) sprite at the
        // alcohol offset.  Skipped for far/small scenery (offset would be
        // sub-pixel) and once the capped pool is spent.
        if (_scnGhostOn && s.visible && (proj.sw ?? 0) >= _SCN_GHOST_MIN_SW
            && _scnGhostUsed < _scnGhostPool.length) {
          const _go = _scnGhostBase * Math.min(1, (proj.sw ?? 0) / 200);
          const _gs = _scnGhostPool[_scnGhostUsed++];
          if (_gs.texture.key !== s.texture.key) _gs.setTexture(s.texture.key);
          _gs.setPosition(s.x + _go, s.y)
            .setDisplaySize(s.displayWidth, s.displayHeight)
            .setDepth(s.depth - 0.01)
            .setAlpha(s.alpha * _scnGhostAlpha)
            .setFlipX(s.flipX)
            .setRotation(s.rotation)
            .setVisible(true);
          if (s.isCropped && s._crop) _gs.setCrop(s._crop.x, s._crop.y, s._crop.width, s._crop.height);
          else _gs.setCrop();
          _gs.setTint(s.tintTopLeft ?? 0xFFFFFF);
        }
      }
    }
    for (let i = used; i < pool.length; i++) pool[i].setVisible(false);
    for (let i = _scnGhostUsed; i < _scnGhostPool.length; i++) _scnGhostPool[i].setVisible(false);
  ...
  _renderViceSprites() {
    ...
    for (let n = 380; n >= 0 && used < pool.length; n--) {
      ...
      for (const sp of seg.sprites) {
        ...
        const _t     = this.gameTime ?? 0;
        const _phase = ((sp.lootSeed ?? Math.abs(sp.offset ?? 0)) % 1) * 6.283;
        const _bob   = Math.sin(_t * 2.4 + _phase) * dispH * 0.12;
        const _cx    = proj.sx;
        const _cy    = proj.sy - dispH * 0.4 - _bob;
        s.setPosition(_cx, _cy)
        ...
        if (ghostOffsetBase > 0 && ghostPool && ghostUsed < ghostPool.length) {
          ...
          gs.setPosition(_cx + ghostOffset, _cy)
            .setDisplaySize(dispW, dispH)
            .setDepth(depth - 0.01)
            .setRotation(Math.sin(_t * 1.6 + _phase) * 0.10)
            .setAlpha(ghostAlpha)
            .setVisible(true);
        }
      }
  ...
  _buildHUD() {
    ...
    }).setOrigin(mox(1), 0).setDepth(d + 1);   // above the wallet ($) text when overlapping
    ...
    this.hudSteroid = this.add.text(SCREEN_W / 2, SCREEN_H - 42, '', {
    ...
    this.hudNarcan = this.add.text(SCREEN_W / 2, SCREEN_H - 58, '', {
    ...
    this._layoutWiperButton = () => {
      ...
      // Base geometry for the Customize-Controls move+scale (applied as a
      // Graphics transform in _applyControlLayout — the icon is drawn at
      // absolute coords, so we scale/position the whole object around it).
      this._wiperBaseLeft = wiperLeft;
      this._wiperBaseTop  = wiperTop;
      this._wiperSize     = muteSize;
    ...
    {
      // Base (resting) size.  Touch-and-HOLD the mirror to zoom it to
      // _mirrorHoldZoom× (see _setMirrorZoom); release returns to 1×.
      // Geometry, frame draw, and mask fill are all factored into helpers
      // so the zoom path recomputes them identically.
      // _mirrorHoldZoom is the SHARED max-expansion config — it is NOT
      // tied to difficulty; the same factor applies in Easy/Normal/Hard/
      // Custom.  Tune the held mirror size here, in one place.
      this._mirrorBaseW = 260;
      this._mirrorBaseH = 56;
      this._mirrorHoldZoom = 3.0;   // max expansion (was 1.5× — doubled per spec)
      this._mirrorZoom  = 1;
      this._mirrorBounds     = this._mirrorGeom(this._mirrorBaseW, this._mirrorBaseH);
      this._mirrorBaseBounds = this._mirrorGeom(this._mirrorBaseW, this._mirrorBaseH);
      // Frame body — outer black housing + light grey trim only.
      this.hudMirrorBg    = this.add.graphics().setDepth(d - 5);
      this._drawMirrorFrame();
      ...
      // edge of a lane.  Re-filled (not recreated) on zoom so the pool
      // sprites keep their reference to this same mask object.
      const maskShape = this.make.graphics({ x: 0, y: 0, add: false });
      ...
      // Mirror fog veil sits ABOVE reflected cars/scenery but BELOW the
      // mirror lamp overlay, so the reflected world is milky while lights
      // still glow through the haze.
      this.hudMirrorFog = this.add.graphics()
        .setDepth(d - 3.30)
        .setMask(this._mirrorMask);
      this._hudObjects?.push(this.hudMirrorFog);

      // Lamp overlay for the mirror.  Road/background glow stays in
      // hudMirrorGlass, but actual bulbs must render ABOVE the mirror car
      // sprites/fog veil or the PNG bodies + fog wash hide them.
      this.hudMirrorLights = this.add.graphics()
        .setDepth(d - 3.25)
        .setMask(this._mirrorMask);
      this._hudObjects?.push(this.hudMirrorLights);
      ...
      // Pool of mirror scenery sprites — buildings/houses/trees/landmarks
      // (cranes) the player has driven past, drawn to MATCH the forward
      // view.  150 slots: dense city rows (Bellevue) + trees + cranes would
      // exhaust a small pool, leaving blank gaps where far buildings "pop
      // in" late and isolated buildings read as floating.  Forward uses 400.
    ...
    // Depth 64 = ABOVE the genre button (bg 62 / icon 63) so the station-name
    // text is never hidden behind the button when they overlap.
    }).setOrigin(0.5, 0).setDepth(64).setInteractive({ useHandCursor: true });
    this.hudRadio.on('pointerdown', (ptr) => {
      // In the controls editor this text is a DRAG handle, not a station
      // switch — don't change the music while repositioning it.
      if (this._ctrlEditMode) return;
      ptr.event?.stopPropagation?.();
      this.audio?.nextStation?.();
    });
    ...
    // Pedals sit side-by-side on the BOTTOM row, flanking the centre
    // mileage·location readout: BRAKE just LEFT of the mileage, ACCEL just
    // RIGHT of the location.  Their X is FIXED (not handedness-mirrored and
    // it never nudges with the live text) — computed from the LONGEST
    // possible readouts ("293 MI" + "SNOQUALMIE PASS") so the pair hugs the
    // centre as tightly as it can while guaranteeing it never overlaps the
    // text.  Origin (0.5, 1) → x is the pedal centre, y its bottom edge.
    const PEDAL_ROW_Y = SCREEN_H - 4;
    this._pedalRowY = PEDAL_ROW_Y;
    {
      const PEDAL_TEXT_GAP = 14;                       // clearance from the readout
      // Measure the LONGEST readouts with the REAL hud text objects — they
      // carry the correct font/style, so this is reliable (an offscreen
      // make.text mismeasured at build time and the pedals crept onto the
      // text).  Floors guarantee clearance even if the web font isn't
      // resolved yet at this instant.
      const measureReal = (obj, longest, floor) => {
        if (!obj?.setText) return floor;
        const prev = obj.text;
        obj.setText(longest);
        const w = obj.width;
        obj.setText(prev);
      ...
      const Wmile   = measureReal(this.hudDist,   '293 MI',          56);
      const Wregion = measureReal(this.hudRegion, 'SNOQUALMIE PASS', 140);
      const MILE_RIGHT_X = SCREEN_W / 2 - 6;           // hudDist right edge (origin 1,1)
      const REGION_CX    = SCREEN_W / 2;               // hudRegion centre  (origin 0.5,1)
      // Left edge of the whole readout cluster (mileage extends left; the
      // centred location extends both ways).
      const leftExtent  = Math.min(MILE_RIGHT_X - Wmile, REGION_CX - Wregion / 2);
      // BRAKE sits a touch further left so it never crowds the mileage's
      // first digit.  ACCEL mirrors BRAKE exactly about the screen centerline
      // (per request) — the location is centred/symmetric, so the mirror
      // clears its right edge by the same margin the brake clears the left.
      const BRAKE_EXTRA = 16;
      this._brakePedalX = leftExtent - PEDAL_TEXT_GAP - BRAKE_EXTRA - PEDAL_W / 2;   // BRAKE (left)
    ...
    gasBtn.on('pointerdown', (ptr) => {
      ...
      // Pedals are mutually exclusive — engaging gas releases the brake.
    ...
    // Solid-black backdrop behind the title art.  On the decoupled-width canvas
    // the title art only covers the centre 800, so without this the live
    // road/scenery shows (and moves) on the widened sides while picking options.
    // It lives on the WORLD camera (added to _worldObjects below, so the UI
    // camera ignores it) at a depth above all scenery, so the whole world goes
    // black behind the UI-camera title art.  Toggled by _setTitleVisible.
    this._titleBlackout = this.add.rectangle(SCREEN_W / 2, SCREEN_H / 2, 3000, SCREEN_H + 800, 0x000000)
      .setDepth(50)
    ...
    const THUMBS_OPTIONS = [
      { id: 'default', label: 'DEFAULT', blurb: 'Auto: shifts with weather' },
    ...
    const DIFF_OPTIONS = [
      { id: 'easy',   label: 'EASY',   blurb: 'Less Cars, Less Cops' },
    ...
    // First-ever defaults: steering DEFAULT (adaptive) and Difficulty
    // EASY (matches Difficulty.DEFAULT_MODE — the title wheel must agree,
    // or tapping Start with no prior pick silently launched Normal).
    ...
    if (thumbsIdx < 0) thumbsIdx = 0;   // default → DEFAULT (adaptive, weather-switching)
    let diffIdx = DIFF_OPTIONS.findIndex(o => o.id === storedDiff);
    if (diffIdx < 0) diffIdx = DIFF_OPTIONS.findIndex(o => o.id === 'easy');
  ...
  _renderHUD() {
    ...
    if (!this._ctrlEditMode && this.hudHPDamage?.visible && this._hpDamageUntil
    ...
    if (this.hudMirrorGlass && this._mirrorBounds && this._mirrorCarPool && !this._perf?.noMirror) {
      ...
      // (double-integral of seg.curve, exactly like the main view's
      // screenDX/screenX), instead of leaning as one straight wedge.  A
      // LEFT turn ahead reads as the road bending RIGHT behind you, so the
      // bend is negated.  Scaled to the glass (grows with zoom) and clamped
      // so a hairpin stays in frame.
      const segsForCurve = this.road?.segments;
      const nSegC        = segsForCurve?.length ?? 0;
      const startBack    = Math.floor(p.position / SEG_LENGTH);
      const MIRROR_FAR_Z = 36000;                       // draw distance (also used by sprites below)
      const ROAD_N       = Math.round(MIRROR_FAR_Z / SEG_LENGTH);   // slices to the vanishing point
      const CURVE_SCALE  = mb.glassW * 0.0045;          // TUNABLE — mirror bend strength
      const MAX_BEND     = mb.glassW * 0.42;            // hairpin clamp (stays inside the glass)
      // PERSPECTIVE depth (1/z) — matches the forward view so rear scenery
      // BUNCHES toward the horizon and spaces/sizes like the real world.
      // (Was linear/even depth, which made the houses behind look far too
      // separated vs. the forward direction.)  depthT: 1 at the near plane,
      // 0 at the horizon.
      const MIRROR_NEAR_Z = 700;                        // near plane (TUNABLE)
      const _invNearM = 1 / MIRROR_NEAR_Z, _invFarM = 1 / MIRROR_FAR_Z;
      const mirrorDepthT = (relZ) => {
        const z = Math.max(MIRROR_NEAR_Z, Math.min(MIRROR_FAR_Z, relZ));
        return (1 / z - _invFarM) / (_invNearM - _invFarM);
      };
      // 1) Per-segment bend in SEGMENT space (double integral of curve, no
      //    wrap past the start) → bendBySeg[n].
      const bendBySeg = new Array(ROAD_N + 1);
      {
        let bendV = 0, bend = 0;
        for (let n = 0; n <= ROAD_N; n++) {
          if (n > 0 && nSegC) {
            const realIdx = startBack - n;              // segment n BEHIND the player
            if (realIdx >= 0) {                         // never wrap past the route START
              bendV += segsForCurve[realIdx % nSegC]?.curve ?? 0;   // 1st integral → slope
              bend  += bendV;                                       // 2nd integral → offset
            }
            // realIdx < 0 → nothing behind the start; freeze the bend (straight)
          }
          bendBySeg[n] = bend;
        }
      }
      // Screen-x bend offset of the centerline at segment depth nf (frac ok),
      // negated for the rear view + clamped.  Shared by the road draw and the
      // sprite projector so they stay on one curve.
      const bendOffAtSeg = (nf) => {
        const f = Math.max(0, Math.min(ROAD_N, nf));
        const i = Math.floor(f), frac = f - i;
        const bend = bendBySeg[i] + (bendBySeg[Math.min(ROAD_N, i + 1)] - bendBySeg[i]) * frac;
        return Math.max(-MAX_BEND, Math.min(MAX_BEND, -bend * CURVE_SCALE));
      };
      // 2) Inverse perspective: screen depthT → world relZ.
      const relZfromDepthT = (dT) => 1 / (dT * (_invNearM - _invFarM) + _invFarM);
      // 3) Sample the centerline UNIFORMLY in screen depth (k=0 near/bottom →
      //    k=ROAD_SAMPLES at the horizon).  Uniform screen spacing keeps the
      //    perspective road SMOOTH near the camera (segment-spaced sampling
      //    would facet badly there, since perspective changes fastest up
      //    close).
      const ROAD_SAMPLES = 48;
      const roadCenter = new Array(ROAD_SAMPLES + 1);
      for (let k = 0; k <= ROAD_SAMPLES; k++) {
        const depthT = 1 - k / ROAD_SAMPLES;            // 1 = near (bottom), 0 = horizon
        const nf     = relZfromDepthT(depthT) / SEG_LENGTH;
        roadCenter[k] = {
          depthT,
      ...
      if (inTunnel) {
        ...
        // curved road's left edge (not a straight vertical line), so the
        // shoreline tracks the road through bends instead of the road
        // crossing a flat water rectangle.
        mg.fillStyle(grass, 1);
        mg.fillRect(mb.glassX, groundY, mb.glassW, groundH);
        mg.fillStyle(water, 1);
        const wp = [{ x: mb.glassX, y: groundY }];
        for (let n = ROAD_SAMPLES; n >= 0; n--) {         // far→near along the road's LEFT edge
          const s = roadCenter[n];
          wp.push({ x: s.cx - s.halfW, y: s.y });
        }
      ...
      mg.fillStyle(palette.road2 ?? palette.road1 ?? 0x2A2A2A, 1);
      {
        ...
        for (let n = 0; n <= ROAD_SAMPLES; n++) {       // uniform screen samples → smooth
          const s = roadCenter[n];
          left.push({ x: s.cx - s.halfW, y: s.y });
          right.push({ x: s.cx + s.halfW, y: s.y });
        }
        right.reverse();
        mg.fillPoints(left.concat(right), true);
      }
      // Horizon line.
      ...
      const stripeCurve = (xRatio, color, alpha) => {
        ...
        mg.beginPath();
        let started = false;
        for (let n = 0; n <= ROAD_SAMPLES; n++) {
          const s = roadCenter[n];
          const x = s.cx + xRatio * s.halfW;
          if (!started) { mg.moveTo(x, s.y); started = true; } else mg.lineTo(x, s.y);
        }
        mg.strokePath();
      };
      const dashedStripeCurve = (xRatio, color, alpha) => {
        mg.lineStyle(1, color, alpha);
        // Draw every other short span so it reads as a broken lane line.
        for (let n = 0; n < ROAD_SAMPLES; n += 2) {
          const a = roadCenter[n];
          const b = roadCenter[Math.min(ROAD_SAMPLES, n + 1)];
          mg.beginPath();
          mg.moveTo(a.cx + xRatio * a.halfW, a.y);
          mg.lineTo(b.cx + xRatio * b.halfW, b.y);
          mg.strokePath();
        }
      };
      ...
      const placeSprite = (s, tex, x, y, depthT, maxH = 26) => {
        ...
        // Scale by the live mirror zoom so sprites GROW with the glass on
      ...
      // (MIRROR_FAR_Z is declared up with the curved-road sampler above.)
      // Scenery sprites behind the player — drawn to MATCH the forward view
      // (_renderSceneSprites): same textured types (buildings, houses,
      // trees, desert brush AND landmarks like the West Seattle cranes,
      // which the old filter dropped), and sized off the mirror road WIDTH
      // × the SAME per-sprite multiplier the forward renderer uses
      // (SCENERY_IMAGE_PROFILES heightMult, else a type default), so
      // proportions read identically to what's ahead.
      const SCENE_TYPES = new Set(['building', 'house', 'tree', 'landmark', 'cactus', 'palm', 'shrub']);
      // The mirror is a smaller window onto the same world — scale the
      // forward per-profile max height into the panel so a sprite caps at
      // the SAME fraction of the view it does on the main screen.
      const viewScale = mb.glassH / SCREEN_H;
      const placeSceneSprite = (s, tex, x, groundY, depthT, sp) => {
        if (s.texture.key !== tex && this.textures.exists(tex)) s.setTexture(tex);
        const t  = this.textures.get(s.texture.key).source[0];
        const bw = t?.width || 64, bh = t?.height || 64;
        const profile  = SCENERY_IMAGE_PROFILES[tex];
        const isTree     = sp.type === 'tree' || sp.type === 'cactus' || sp.type === 'palm' || sp.type === 'shrub';
        const isLandmark = sp.type === 'landmark';
        const sizeMult = sp.sizeMult ?? (isLandmark ? 5.5 : isTree ? 2.0 : 2.6);
        const mult = profile?.heightMult
          ?? (profile?.widthMult ? profile.widthMult * (bh / bw) : sizeMult);
        // Mirror road WIDTH at this depth == forward proj.sw analogue
        // (2 × half-width).  Same mult → same fraction of the road → parity.
        let targetH = (2 * mb.roadHalfW * depthT) * mult * (sp.heightBoost ?? 1);
        if (targetH < 1.2) { s.setVisible(false); return false; }   // sub-pixel cull
        const capH = (profile?.maxH ?? SCREEN_H * 0.85) * viewScale;
        if (targetH > capH) targetH = capH;                         // clamp (keeps aspect via targetW)
        const targetW = targetH * (bw / bh);
        s.setDisplaySize(targetW, targetH);
        s.setPosition(x, groundY);                  // origin (0.5,1) → base sits on the road's ground line
        s.setVisible(true);
        return true;
      };
      ...
      if (segs?.length) {
        ...
        for (let n = 1; n <= 180 && usedBuildings < buildingPool.length; n++) {
          const realIdx = visualSegIdx - n;
          if (realIdx < 0) break;                 // no scenery behind the route START (never wrap to Pullman)
          const seg = segs[realIdx % segs.length];
          if (!seg?.sprites) continue;
          for (const sp of seg.sprites) {
            if (sp.collected || !sp.texKey) continue;
            if (sp.isCollectible || sp.copEncounter) continue;
            if (!SCENE_TYPES.has(sp.type)) continue;
            // Structure setback — match the forward minOffset so buildings
            // sit at the same lateral distance from the road.
            let visualOffset = sp.offset ?? 0;
            if (sp.type === 'building' || sp.type === 'house') {
              const profile = SCENERY_IMAGE_PROFILES[sp.texKey];
              const minOffset = Math.max(profile?.minOffset ?? 0, sp.visualMinOffset ?? 1.05);
              const sign = visualOffset >= 0 ? 1 : -1;
              visualOffset = sign * Math.max(Math.abs(visualOffset), minOffset);
            }
            const vz   = n * SEG_LENGTH + SEG_LENGTH / 2;
            const proj = projectRear(vz, visualOffset, MIRROR_FAR_Z);
            const s    = buildingPool[usedBuildings];
            if (!placeSceneSprite(s, sp.texKey, proj.x, proj.y, proj.depthT, sp)) continue;
            usedBuildings++;
            // Scenery PNGs are authored right-side; left-side instances flip.
            const autoFlipLeft = (sp.type === 'building' || sp.type === 'house') && visualOffset < 0;
      ...
      // vz is measured back from the player CAR SPRITE (at car-visual).
      // The ONLY thing we must exclude is what's ahead of you (vz <= 0).
      // Beyond a tiny floor — so a vehicle sitting essentially AT your own
      // position (vz ≈ 0, i.e. "you") doesn't render — everything behind
      // shows, as close as it really is.  Size can't blow up: the mirror's
      // MIRROR_NEAR_Z perspective clamp caps how big a near vehicle draws.
      // (Old value was a full PLAYER_VIRTUAL_Z = 3000, which hid the entire
      ...
      for (const { c: car, vz } of carsBehind) {
        ...
        if (_hlOnMirror) {
          ...
          if (!wasOncoming) {
            ...
            ml.fillStyle(0xFFD850, Math.min(1, lightAM * MIRROR_HL_BOOST));
            ml.fillCircle(proj.x - grilleDx, lampY, haloR);
            ml.fillCircle(proj.x + grilleDx, lampY, haloR);
            ml.fillStyle(0xFFE680, Math.min(1, lightAM * 1.4 * MIRROR_HL_BOOST));
            ml.fillCircle(proj.x - grilleDx, lampY, coreR);
            ml.fillCircle(proj.x + grilleDx, lampY, coreR);
            ...
            ml.blendMode = Phaser.BlendModes.ADD;
            ml.fillStyle(0xFF2A1A, lightAM);
            ml.fillCircle(proj.x - tailDx, tailY, haloR);
            ml.fillCircle(proj.x + tailDx, tailY, haloR);
            ml.fillStyle(0xFF5544, Math.min(1, lightAM * 1.4));
            ml.fillCircle(proj.x - tailDx, tailY, coreR);
            ml.fillCircle(proj.x + tailDx, tailY, coreR);
            ml.blendMode = Phaser.BlendModes.NORMAL;
          }
        }
      ...
      // Mirror fog veil — the mirror paints its own rear scene, so it must get
      // its own fog wash.  This sits above reflected cars/scenery but below the
      // mirror light overlay, matching the real view: the world is milky, lights
      // still punch through as soft blooms.
      if (this.hudMirrorFog) {
        const fpM = Weather.fogParams?.(_mileMirror);
        const dM  = Math.max(0, Math.min(1, fpM?.density ?? 0));
        if (dM > 0.03) {
          const fg = this.hudMirrorFog;
          const fogCol = lerpColor(skyFog, 0xFFFFFF, 0.42);
          fg.fillStyle(fogCol, Math.min(0.42, dM * 0.34));
          fg.fillRect(mb.glassX, mb.glassY, mb.glassW, mb.glassH);
          // Feathered horizon/road wash.  Avoid a ruler-straight fog shelf by
          // stacking low-alpha, slightly wavy bands plus soft ovals.
          const phase = (this.time?.now ?? 0) * 0.00035;
          const waveBand = (yTop, h, amp, alpha, steps = 8) => {
            const top = [], bot = [];
            for (let i = 0; i <= steps; i++) {
              const t = i / steps;
              const x = mb.glassX + t * mb.glassW;
              const w1 = Math.sin(t * Math.PI * 2.2 + phase) * amp;
              const w2 = Math.sin(t * Math.PI * 2.8 + phase + 1.7) * amp * 0.65;
              top.push({ x, y: yTop + w1 });
              bot.push({ x, y: yTop + h + w2 });
            }
            fg.fillStyle(fogCol, alpha);
            fg.fillPoints(top.concat(bot.reverse()), true);
          };
          const baseY = mb.horizonY - mb.glassH * 0.16;
          waveBand(baseY,                   mb.glassH * 0.22, 1.8, Math.min(0.20, dM * 0.18));
          waveBand(baseY + mb.glassH * 0.09, mb.glassH * 0.31, 2.4, Math.min(0.34, dM * 0.30));
          waveBand(baseY + mb.glassH * 0.23, mb.glassH * 0.42, 1.5, Math.min(0.25, dM * 0.22));
          fg.fillStyle(fogCol, Math.min(0.18, dM * 0.16));
          fg.fillEllipse(mb.glassX + mb.glassW * 0.34, mb.horizonY + mb.glassH * 0.10, mb.glassW * 0.95, mb.glassH * 0.32);
          fg.fillEllipse(mb.glassX + mb.glassW * 0.72, mb.horizonY + mb.glassH * 0.16, mb.glassW * 0.80, mb.glassH * 0.28);
        }
      }
    ...
    if (this._ctrlEditMode) {
      this.hudPopup.setVisible(true).setAlpha(1);   // editor placeholder stays put
    } else {
      this.hudPopup
...
// 5★ helicopter overlay — hovers above the road, sway + bob + rotor.
    ...
    this._applyHudLayout();      // Customize Controls: shift moved readouts last
  ...
  _drawViceIcons() {
    ...
    const buildViceImage = (texKey, cx, cy) => {
      ...
      const fit   = Math.min((46 - 6) / baseW, (42 - 6) / baseH);
      const img = this.add.image(cx, cy, texKey).setOrigin(0.5).setDepth(25);
      img._cellBaseW = baseW * fit;  img._cellBaseH = baseH * fit;
      img.setDisplaySize(img._cellBaseW, img._cellBaseH);
      return img;
    };
    ...
    for (const id of Object.values(VICES)) {
      ...
      } else if (!slot.isImage) {
        slot.icon.setScale(o.s);
      }
      ...
      // ── Card visuals ────────────────────────────────────────────
      if (!unlocked) {
        // Undiscovered → a plain black semi-translucent block placeholder.
   ...
   *  button so both look and behave identically.  `glow` draws the
   *  next-to-be-used highlight ring. */
  _renderF12Cell(tType, x, y, count, glow, scale = 1) {
    const g = this.hudGfx;
    // Touch-target sized cell — squarer (58×56) per UX feedback that the
    // old rectangles read too wide.  `scale` is the Customize-Controls pinch
    // factor for the weapon/disguise cluster (1 = default).
    const baseW0 = 58, baseH0 = 56;
    const iconW = baseW0 * scale, iconH = baseH0 * scale;
    ...
    if (!this._f12Texts[tType.id]) {
      ...
      if (hasImg) {
        ...
        // inside (58-6) × (56-6).  No stretching.  Cache the unscaled fit
        // size so the per-frame scale can re-derive the display size.
        const tex   = this.textures.get(tType.tex)?.source?.[0];
        const baseW = tex?.width  || baseW0;
        const baseH = tex?.height || baseH0;
        const fit   = Math.min((baseW0 - 6) / baseW, (baseH0 - 6) / baseH);
        icon = this.add.image(x + iconW / 2, y + iconH / 2, tType.tex)
          .setOrigin(0.5).setDepth(25);
        icon._cellBaseW = baseW * fit;  icon._cellBaseH = baseH * fit;
      } else {
        icon = this.add.text(x + iconW / 2, y + iconH / 2, tType.label, {
          fontSize: '32px',
        }).setOrigin(0.5).setDepth(25);
        icon._cellEmoji = true;
      }
      // Invisible interactive hit-zone covering the cell — taps fire
      // that weapon (mobile-friendly direct fire).  Created at BASE size; the
      // editor pinch scale is applied via setScale below — NOT setSize, which
      // on a Phaser Rectangle does NOT resize the input hit area, so a scaled
      // icon's taps would land outside the (stale) zone and do nothing.
      const hit = this.add.rectangle(
        x + iconW / 2, y + iconH / 2, baseW0, baseH0, 0x000000, 0,
    ...
    if (txt.icon._cellEmoji) txt.icon.setScale(scale);
    else txt.icon.setDisplaySize(txt.icon._cellBaseW * scale, txt.icon._cellBaseH * scale);
    txt.hit?.setPosition(x + iconW / 2, y + iconH / 2);
    txt.hit?.setScale?.(scale);   // scales the hit AREA (transform-applied), unlike setSize
    // Count badge anchored to bottom-right INSIDE the cell.
    txt.count.setPosition(x + iconW - 3 * scale, y + iconH - 2 * scale);
    txt.count.setScale(scale);
    txt.icon.setAlpha(count > 0 ? 1 : 0.18);
    txt.count.setText(count > 0 ? `×${count}` : '');

    // Card background
    g.fillStyle(tType.color, count > 0 ? 0.55 : 0.12);
    g.fillRoundedRect(x, y, iconW, iconH, 4 * scale);
    g.lineStyle(1, 0x000000, 0.6);
    g.strokeRoundedRect(x, y, iconW, iconH, 4 * scale);

    // Glow on the next-to-be-used token
    if (glow) {
      g.lineStyle(2, tType.color, 1);
      g.strokeRoundedRect(x - 1, y - 1, iconW + 2, iconH + 2, 5 * scale);
    }
  }

  _drawF12Inventory() {
    const tokens = this.cops.f12Tokens;
    // Title screen — icons are hidden by _setHudVisible(false).  Exception: the
    // controls editor shows the (empty) slots so they can be repositioned.
    if (this._awaitingStart && !this._ctrlEditMode) return;

    // Four deployable weapons stack against the player's dominant-thumb
    ...
    // left in left-handed mode.  Each weapon is INDEPENDENTLY movable+scalable
    // (id `weapon_<id>`): its default slot is the column row, then its own
    // offset/scale applies on top.
    const lhWeap   = !!this._leftHanded;
    const xLeft    = 10;
    const xRight   = SCREEN_W - 10;
    const baseColX = lhWeap ? xLeft : (xRight - 58);
    const baseYTop = 65, baseRow = 56 + 6;   // unscaled cell + gap
    ...
    for (const w of WEAPONS) {
      ...
      const bx = baseColX + o.dx;
      const by = baseYTop + row * baseRow + o.dy;
      this._renderF12Cell(w, bx, by, count, count > 0 && w.id === topTok, o.s);
      this._weaponCellBounds[w.id] = { x: bx, y: by, w: 58 * o.s, h: 56 * o.s };
      row++;
    }
  ...
  _collectSaveSnapshot(stopId) {
    ...
    const save  = this.registry?.get?.('save');
    const acc   = (save?.get?.('accessories') ?? {})[vid] ?? {};
    ...
    return {
      ...
      runState: {
        ...
        noDamageFlags: { ...(this._noDamageFlags ?? {}) },
        peakStars: Math.max(0, Number(this._peakStars) || 0),
        fiveStarSurvived: !!this._fiveStarSurvived,
        everHitStars: !!this._everHitStars,
        everUsedRestStop: !!this._everUsedRestStop,
        lastMixedLineMile: Math.max(0, Math.round(this._lastMixedLineMile ?? 0)),
        beerLineTimer: Math.max(0, Number(this._beerLineTimer) || 0),
        bonusWeaponTimer: Math.max(0, Number(this._bonusWeaponTimer) || 0),
      },
      messageState: {
        buddyThreads,
        buddyTextTimer: Math.max(0, Number(this._buddyTextTimer) || 0),
        trapWarnSent: Array.from(this._trapWarnSent ?? []),
        girlThread: cleanThread(this._girlThread),
        girlTextPending: !!this._girlTextPending,
        girlStreak: Math.max(0, Math.round(this._girlStreak ?? 0)),
      },
    };
  }
  ...
  _applyMessageState(m) {
    ...
    if (Number.isFinite(m.buddyTextTimer)) this._buddyTextTimer = Math.max(0, m.buddyTextTimer);
    if (Array.isArray(m.trapWarnSent)) this._trapWarnSent = new Set(m.trapWarnSent);
    if (Array.isArray(m.girlThread)) this._girlThread = cleanThread(m.girlThread);
    if (typeof m.girlTextPending === 'boolean') this._girlTextPending = m.girlTextPending;
    if (Number.isFinite(m.girlStreak)) this._girlStreak = Math.max(0, Math.round(m.girlStreak));
  }
  ...
  _applyRunState(r) {
    ...
    if (r.noDamageFlags && typeof r.noDamageFlags === 'object') {
      this._noDamageFlags = {
        '1m': r.noDamageFlags['1m'] === true,
        '2m': r.noDamageFlags['2m'] === true,
        '3m': r.noDamageFlags['3m'] === true,
        '5m': r.noDamageFlags['5m'] === true,
      };
    }
    if (Number.isFinite(r.peakStars)) this._peakStars = Math.max(0, r.peakStars);
    if (typeof r.fiveStarSurvived === 'boolean') this._fiveStarSurvived = r.fiveStarSurvived;
    if (typeof r.everHitStars === 'boolean') this._everHitStars = r.everHitStars;
    if (typeof r.everUsedRestStop === 'boolean') this._everUsedRestStop = r.everUsedRestStop;
    if (Number.isFinite(r.lastMixedLineMile)) this._lastMixedLineMile = Math.max(0, Math.round(r.lastMixedLineMile));
    if (Number.isFinite(r.beerLineTimer)) this._beerLineTimer = Math.max(0, r.beerLineTimer);
    if (Number.isFinite(r.bonusWeaponTimer)) this._bonusWeaponTimer = Math.max(0, r.bonusWeaponTimer);
  }

  /** Apply a decoded/loaded snapshot to the freshly-started run.  Tolerant
   *  of missing fields (legacy partial snapshots) so it never throws. */
  _applyResumeSnapshot(s) {
    if (!s) return;
    try {
      if (typeof s.score === 'number') this.score = s.score;
      // Vehicle FIRST (swaps sprite + resets damage max), then override HP.
      if (s.vehicleId && VEHICLES[s.vehicleId]) {
        if (Array.isArray(s.owned)) this.registry?.set?.('ownedVehicles', s.owned);
        const save = this.registry?.get?.('save');
        if (save && s.accessories) {
          const accAll = save.get('accessories') ?? {};
          accAll[s.vehicleId] = { ...(accAll[s.vehicleId] || {}), ...s.accessories };
          save.set('accessories', accAll);
        }
        if (this.player) this.player.vehicleId = s.vehicleId;
      ...
      // a code carrying radar arms it for this run AND persists it globally.
      if (s.radar) {
        this._hasRadar = true;
  ...
  _saveRestStop(stopId, code, snap) {
    try {
      ...
      // Mirror the checkpoint to the cloud (best-effort, fire-and-forget) so it
      // can be pulled on any device — no copy/paste code needed.  Keyed by the
      // slot's stable playerId; offline / pre-deploy this no-ops silently.
      try {
        CloudSave.put({
  ...
  _setTitleVisible(v) {
    this._titleBlackout?.setVisible(v);
    [
      this._titleScrim, this._titleBackdrop, this._titleMain, this._titleSub, this._titleRoute, this._titleTap,
      this._titleResume,    this._titleResumeTxt,
      this._titleEnterCode, this._titleEnterCodeTxt,
  ...
  // Freeze the sim, show the HUD, make every registered HUD element draggable,
  // and float a centered panel (DRAG TO MOVE • RESET • UNDO • DONE).  Each
  // element's screen offset is stored per-profile in _hudLayout and re-applied
  // every frame by _applyHudLayout (at the end of _renderHUD).  Built on the UI
  // camera so the panel sits centered + on top.

  /** [id, [gameObjects]] for every movable HUD GROUP.  Multi-object groups
   *  (speed+MPH, clock+multiplier) move as one unit; the gas pump icon and its
   *  readout are SEPARATE so each can be placed on its own.  id is the stable
   *  save key.  Missing objects are filtered so the group still works. */
  _hudMovableGroups() {
    return [
      ['score',   [this.hudScore]],
  ...
  /** Apply each MOVED group's offset + pinch SCALE.  Members keep their own
   *  captured base position/scale; un-moved groups are left alone so their
   *  normal (sometimes dynamic) positioning is preserved. */
  _applyHudLayout() {
    if (!this._hudLayout) return;
    for (const [id, objs] of this._hudMovableGroups()) {
      for (const obj of objs) {
        if (obj._hudBaseX === undefined) {
          obj._hudBaseX = obj.x;  obj._hudBaseY = obj.y;
          obj._hudBaseSX = obj.scaleX;  obj._hudBaseSY = obj.scaleY;
        }
      }
      const o = this._hudLayout[id];
      if (!o) continue;                       // un-moved → leave default positioning
      // Scale the group AS A UNIT around its anchor (member 0), so the spacing
      // between members scales too (e.g. MPH stays below the grown speed number
      // instead of being swallowed by it).
      const dx = o.dx || 0, dy = o.dy || 0, s = o.scale || 1;
      const ax = objs[0]._hudBaseX, ay = objs[0]._hudBaseY;
      for (const obj of objs) {
        obj.x = ax + dx + (obj._hudBaseX - ax) * s;
        obj.y = ay + dy + (obj._hudBaseY - ay) * s;
        obj.setScale(obj._hudBaseSX * s, obj._hudBaseSY * s);
      }
    }
  }
   ...
   *  offset position).  s is clamped like the readout pinch. */
  _ctrlOff(id) {
    const o = this._hudLayout?.[id];
    return { dx: o?.dx || 0, dy: o?.dy || 0, s: Math.max(0.3, Math.min(4, o?.scale || 1)) };
  }

  /** Per-frame positioner for the non-readout controls.  Runs right after
   *  _applyHudLayout (which owns the text readouts).  Bases are RECOMPUTED from
   *  handedness each frame (never captured), so a moved control is pinned to its
  ...
  _applyControlLayout() {
    ...
    const lh  = !!this._leftHanded;
    const top = 2;
    // ── Top-row buttons (pause / ff / genre / mute / map / garage) ──
    if (this._topRowButtons) {
      for (const btn of this._topRowButtons) {
        if (!btn.lbl) continue;
        const o     = this._ctrlOff('btn_' + btn.id);
        const baseX = lh ? btn.baseLeft : (SCREEN_W - btn.baseLeft - btn.size);
  ...
  /** Recompute the mirror frame/mask/bounds when its custom offset or scale
   *  changes (only on change — the mask + frame redraw is not free).  Offset is
   *  applied inside _mirrorGeom; scale multiplies the base size alongside the
   *  hold-to-zoom factor. */
  _applyMirrorOffset() {
    if (!this._mirrorBaseW || !this.hudMirrorBg) return;
    const o = this._ctrlOff('mirror');
    if (this._mirrorUserDx === o.dx && this._mirrorUserDy === o.dy && this._mirrorUserScale === o.s) return;
    this._mirrorUserDx = o.dx;  this._mirrorUserDy = o.dy;  this._mirrorUserScale = o.s;
    const z = this._mirrorZoom || 1;
    this._mirrorBaseBounds = this._mirrorGeom(this._mirrorBaseW * o.s,       this._mirrorBaseH * o.s);
    this._mirrorBounds     = this._mirrorGeom(this._mirrorBaseW * o.s * z,   this._mirrorBaseH * o.s * z);
    this._drawMirrorFrame();
    this._refillMirrorMask();
  }

  /** Push the group's CURRENT full state onto the undo stack (before a drag or
   *  pinch changes it). */
  _pushUndo(id) {
    const prev = this._hudLayout[id] || {};
    this._hudUndoStack.push({ id, dx: prev.dx || 0, dy: prev.dy || 0, scale: prev.scale || 1 });
    if (this._hudUndoStack.length > 50) this._hudUndoStack.shift();
  }

  /** The id of the movable group whose bounds contain HUD-space point (hx,hy),
   *  or null.  Used to pick the pinch target. */
  _groupUnderPoint(hx, hy) {
    for (const [id, objs] of this._hudMovableGroups()) {
      for (const obj of objs) {
        const b = obj.getBounds?.();
        if (b && hx >= b.x && hx <= b.x + b.width && hy >= b.y && hy <= b.y + b.height) return id;
      }
    }
    return null;
  }

  /** Editor grab (press, NO movement required — so you can hold finger 1 still
   *  and scale with finger 2).  Sets the held item + makes THIS finger the
   *  position finger.  Ignored if another finger is already mid-gesture (that
   *  finger is the scaler, not a new grab).  All movement + scaling is then run
   *  per-frame by _updateControlsEditor (the finger-1-move / finger-2-scale /
   *  lift-finger-1→finger-2-moves state machine). */
  _onHudGrab(pointer, obj) {
    if (!this._ctrlEditMode || !obj || obj._editGroup === undefined) return;
    if (this._dragGroup && this._posPointer && this._posPointer.isDown
        && pointer !== this._posPointer) return;   // 2nd finger = scaler, don't re-grab
    this._dragGroup  = obj._editGroup;
    this._posPointer = pointer;     // this finger now controls position
    this._posAnchor  = null;        // anchored on the first _updateControlsEditor frame
    this._scaleAnchor = null;
    this._pushUndo(obj._editGroup);
  }
  ...
  _buildControlProxies() {
    ...
    const mk = (id, b) => {
      ...
      // Invisible by default (still fully interactive — input ignores alpha).
      ...
      proxy._editGroup  = id;
      // Position/size are driven each frame by _syncControlProxies from the
      // control's live bounds; the grab+move+scale gesture is run by
      // _updateControlsEditor.  (setInteractive in the rect ctor is enough for
      // gameobjectdown to fire — no drag handlers needed.)
      this.cameras?.main?.ignore?.(proxy);   // UI camera only
      this._ctrlProxies.push(proxy);
    };

    // Top-row buttons
    for (const btn of this._topRowButtons || []) {
      if (btn.lbl && btn._lsz) mk('btn_' + btn.id, { x: btn._lx, y: btn._ly, w: btn._lsz, h: btn._lsz });
    }
    // Pedals (BRAKE, ACCEL) from the published steer-exclusion zones
    const pz = this._pedalHitZones || [];
    if (pz[0]) mk('pedalBrake', pz[0]);
    if (pz[1]) mk('pedalGas',   pz[1]);
   ...
   *  each frame, so the handle tracks drags AND pinch-scales exactly.  Uses
   *  displayWidth/Height — which rescales the rendered rectangle; setSize only
   *  changes the logical size and leaves the drawn shape unchanged (that was
   *  the "handles don't grow with the images" bug). */
  _syncControlProxies() {
    if (!this._ctrlProxies || !this._ctrlProxies.length) return;
    const boundsFor = (id) => {
      if (id.startsWith('btn_')) {
        const btn = this._topRowButtons?.find(b => 'btn_' + b.id === id);
        return (btn && btn._lsz != null) ? { x: btn._lx, y: btn._ly, w: btn._lsz, h: btn._lsz } : null;
      }
      if (id === 'pedalBrake') return this._pedalHitZones?.[0];
      if (id === 'pedalGas')   return this._pedalHitZones?.[1];
      if (id === 'mirror')     return this._mirrorBaseBounds;
    ...
    for (const px of this._ctrlProxies) {
      const b = boundsFor(px._editGroup);
      if (!b || !(b.w > 0) || !(b.h > 0)) continue;
      px.setPosition(b.x + b.w / 2, b.y + b.h / 2);
      px.displayWidth  = Math.max(24, b.w);
      px.displayHeight = Math.max(24, b.h);
      // Only the handle being dragged shows its outline; the rest stay invisible
      // (but interactive) so the screen isn't cluttered with boxes.
      const active = (px._editGroup === this._dragGroup);
      px.setFillStyle(0x39A8FF, active ? 0.18 : 0.0);
      px.setStrokeStyle(2.5, 0x39A8FF, active ? 1 : 0);
    }
  }

  _destroyControlProxies() {
    if (this._ctrlProxies) { for (const p of this._ctrlProxies) p?.destroy?.(); }
    this._ctrlProxies = null;
  }

  /** Show the transient pop-ups with example text while editing so they can be
   *  dragged.  Cleared on DONE; in-game they fire at the placed position. */
  _showEditorPopupPlaceholders() {
    ...
    this.hudMult?.setText('×2.0').setColor('#44FF88').setVisible(true);
  }
  ...
  _undoControlsLayout() {
    const last = this._hudUndoStack.pop();
    if (!last) return;
    const moved = last.dx || last.dy || (last.scale && last.scale !== 1);
    if (moved) this._hudLayout[last.id] = { dx: last.dx, dy: last.dy, scale: last.scale };
    else       delete this._hudLayout[last.id];   // back to default
    if (!moved) {
      const grp = this._hudMovableGroups().find(e => e[0] === last.id);
      if (grp) for (const obj of grp[1]) if (obj._hudBaseX !== undefined) {
        obj.x = obj._hudBaseX;  obj.y = obj._hudBaseY;
        obj.setScale(obj._hudBaseSX, obj._hudBaseSY);
      }
    }
    this._applyHudLayout();      // re-applies the moved readout group live
    this._applyControlLayout();  // …and the non-readout controls
    if (this._ctrlEditMode) this._buildControlProxies();   // re-seat the handles
    this._saveHudLayout();
  }

  _enterControlsEditor() {
    if (this._ctrlEditMode) return;
    this._ctrlEditMode = true;
    if (this._paused) this._togglePause?.();    // never edit under the pause overlay
    // If launched from the title, hide the title art so only the HUD shows for
    // editing; remember so DONE can restore the title.
    this._editorFromTitle = !!this._awaitingStart;
    if (this._editorFromTitle) this._setTitleVisible?.(false);
    this._setHudVisible?.(true);                // everything draggable must be visible
    ...
    // Make every movable group member draggable (grabbing any member drags the
    // whole group).
    for (const [gid, objs] of this._hudMovableGroups()) {
      for (const obj of objs) {
        if (obj._hudBaseX === undefined) {
          obj._hudBaseX = obj.x;  obj._hudBaseY = obj.y;
          obj._hudBaseSX = obj.scaleX;  obj._hudBaseSY = obj.scaleY;
        ...
        obj._editGroup    = gid;
        obj._hudEditDepth = obj.depth;     // restore on DONE
        obj.setDepth(65);                  // above the HUD buttons so it always grabs (fixes the clock)
        try { obj.setInteractive({ useHandCursor: true, draggable: true }); } catch (_) {}
      }
    }
    // Grab on PRESS (gameobjectdown, no movement needed); all move+scale is run
    // per-frame by _updateControlsEditor so finger-1 can hold still while
    // finger-2 scales, and finger-2 can take over movement when finger-1 lifts.
    this.input.on('gameobjectdown', this._onHudGrab, this);
    // Centered control panel (keep-clear zone): DRAG TO MOVE + RESET/UNDO/DONE.
    const D = 70;
    ...
    // Disable every OTHER interactive HUD object (pause/FF/music/mute/map/
    // garage, etc.) so nothing fires while editing AND so they stop
    ...
    for (const o of this._hudObjects || []) {           // HUD buttons only (depth-raise keeps draggables on top)
      ...
      if (o._editGroup !== undefined) continue;         // a draggable group member
      if (mk.includes(o)) continue;                     // the editor's own panel
  ...
  /** Per-frame editor gesture machine (Brendan's spec):
   *   • Finger 1 (the grab finger) MOVES the held item.
   *   • Finger 2 anywhere SCALES by the distance between the fingers — pull
   *     apart = grow, push together = shrink.
   *   • Lift finger 1 while finger 2 is down → finger 2 TAKES OVER movement, so
   *     you can place the item precisely without your finger covering it.
   *   • Lift all fingers → committed.
   *  Position uses an anchor-delta (re-anchored whenever the position finger
   *  changes) so the hand-off never jumps. */
  _updateControlsEditor(_dt) {
    if (!this._ctrlEditMode) return;
    const id = this._dragGroup;
    if (!id) { this._posPointer = null; this._posAnchor = null; this._scaleAnchor = null; return; }
    const p1 = this.input.pointer1, p2 = this.input.pointer2;
    const d1 = !!p1?.isDown, d2 = !!p2?.isDown;
    if (!d1 && !d2) {                       // all fingers up → place it + end
      this._dragGroup = null; this._posPointer = null;
      this._posAnchor = null; this._scaleAnchor = null;
      return;
    }
    // Position finger: keep the current one until it lifts, then hand off to
    // whichever finger is still down (re-anchor so there's no jump).
    let posP = this._posPointer;
    if (!posP || !posP.isDown) {
      posP = d1 ? p1 : (d2 ? p2 : null);
      this._posPointer = posP;
      this._posAnchor  = null;
    }
    if (!posP) return;
    // Scale finger: the OTHER finger, only while BOTH are down.
    const scaleP = (d1 && d2) ? ((posP === p1) ? p2 : p1) : null;
    if (!scaleP) this._scaleAnchor = null;

    const cur = this._hudLayout[id] || {};
    let dx = cur.dx || 0, dy = cur.dy || 0, scale = cur.scale || 1;

    // ── Move (anchor-delta — hand-off never jumps) ──
    const cam  = this._uiCam || this.cameras.main;
    const pt   = cam.getWorldPoint(posP.x, posP.y);
    const minX = -C.HUD_OFFSET_X + 4, maxX = SCREEN_W + C.HUD_OFFSET_X - 4;
    const wx = Math.max(minX, Math.min(maxX, pt.x));
    const wy = Math.max(4, Math.min(SCREEN_H - 4, pt.y));
    if (!this._posAnchor) this._posAnchor = { px: wx, py: wy, dx, dy };
    dx = Math.round(this._posAnchor.dx + (wx - this._posAnchor.px));
    dy = Math.round(this._posAnchor.dy + (wy - this._posAnchor.py));

    // ── Scale (fingers apart = grow, together = shrink) ──
    if (scaleP) {
      const dist = Math.hypot(scaleP.x - posP.x, scaleP.y - posP.y) || 1;
      if (!this._scaleAnchor) this._scaleAnchor = { dist, s: scale };
      else scale = Math.max(0.3, Math.min(4, this._scaleAnchor.s * (dist / this._scaleAnchor.dist)));
    }

    this._hudLayout[id] = { dx, dy, scale };
    this._applyHudLayout();
    this._applyControlLayout();
    this._syncControlProxies();
  }

  _exitControlsEditor() {
    this._ctrlEditMode = false;
    this._dragGroup = null;  this._posPointer = null;  this._posAnchor = null;  this._scaleAnchor = null;
    this._destroyControlProxies();   // tear down the non-readout control handles
    ...
    // + restore their normal render depth.
    this.input.off('gameobjectdown', this._onHudGrab, this);
    for (const [, objs] of this._hudMovableGroups()) for (const obj of objs) {
      obj._editGroup = undefined;
      if (obj._hudEditDepth !== undefined) { obj.setDepth(obj._hudEditDepth); obj._hudEditDepth = undefined; }
      try { obj.disableInteractive(); } catch (_) {}
    }
    // Re-enable the HUD buttons we suspended on entry (hit areas were kept).
    for (const o of this._editorDisabledInputs || []) { if (o && o.input) o.input.enabled = true; }
    this._editorDisabledInputs = null;
    this._saveHudLayout();
    if (this._ctrlEditObjs) {
      for (const o of this._ctrlEditObjs) o.destroy?.();
      this._ctrlEditObjs = null;
    }
    // Restore the title if we launched from it (HUD goes back to hidden).
    if (this._editorFromTitle) {
      this._setTitleVisible?.(true);
      this._setHudVisible?.(false);
      this._editorFromTitle = false;
    }
  }
  ...
  _resumeFromSavedSnapshot(snap) {
    ...
    this.scene.start('Game', {
      ...
      resumeSnapshot: snap,         // full state → restored in the resume branch
    });
  }
  ...
  _promptForCode(defaultCode = '') {
    ...
    //           resumed at the EXACT spot.
    //   SAVED — the last deliberately-saved checkpoint (rest-stop save).
    //   <code>— a portable save code, decoded and ported to that spot.
    const save = this.registry.get('save');
    const liveSnap  = this._titleResumeSnap
                   || ((save?.get?.('liveRun')?.snap?.position != null) ? save.get('liveRun').snap : null);
    ...
    const locLine = (snap) => {
      ...
      return `${cp?.name || 'West Seattle'} · Mile ${mi} of ${total}`;
    };
    const lines = [];
    ...
    this._buildCodeEntryPopup((liveSnap || savedSnap) ? '' : defaultCode, (raw) => {
      ...
      if (!code) {
        // Blank RESUME defaults to the in-progress run when there is one — the
        // common case — so the player doesn't have to type LAST.
        if (liveSnap) { this._resumeFromLiveSnapshot(liveSnap); return { ok: true }; }
        return { ok: false, error: 'Type LAST, SAVED, or a save code.' };
      }
      if (kw === 'last') {
        if (liveSnap) { this._resumeFromLiveSnapshot(liveSnap); return { ok: true }; }
        return { ok: false, error: 'No in-progress run to resume.' };
      }
      if (kw === 'saved') {
  ...
  _buildConfirmPopup(title, message, onYes, onNo) {
    ...
    // Invisible input-blocker (no grey film — per Brendan, matched to the
    // pause screen).  Alpha 0 keeps Phaser hit-testing alive so taps can't
    // fall through to the road / pause buttons behind the dialog; widened well
    // past the decoupled canvas so the extended sides are blocked too.
  ...
  _autosaveRun() {
    try {
      ...
      if (this._finishCinematic || this._finishCineEnded
          || this._gameFinished || this._restartModalOpen) return;
      const save = this.registry.get('save');
      if (!save) return;
      const snap = this._collectSaveSnapshot(null);
      if (!snap || (snap.position ?? 0) <= 50) return;   // still at the start line
      const prev = save.get?.('liveRun');
      const keepManual = !!prev?.manual && prev.snap
        && Math.abs((prev.snap.position ?? -999999) - (snap.position ?? 0)) < 5000
        && Math.abs((prev.snap.score ?? -999999) - (snap.score ?? 0)) < 2;
      save.set('liveRun', { snap, ts: Date.now(), manual: keepManual });
    } catch (_) {}
  }

  /** Drop the live-run autosave (clean trip-end / Start Over / Main Menu /
   *  death) so the next boot shows the title instead of resuming a dead run. */
  _clearLiveRun() {
    try { this.registry.get('save')?.set?.('liveRun', null); } catch (_) {}
  }

  /** Manual SAVE (phone-menu "Save" button) — capture the EXACT current spot +
   *  full state to the local resume slot AND the cloud, without ending the run.
   *  Returns { ok } so the UI can confirm.  Resume later via boot auto-resume
    ...
    try {
      ...
      try {
        CloudSave.put({                                          // cross-device (best-effort)
          ...
          snapshot: snap, score: snap.score ?? 0, position: snap.position ?? 0,
        });
      } catch (_) {}
      return { ok: true };
  ...
  /** "Sorry, we lost you…" modal shown after an auto-resume.  Holds the world
   *  frozen (via _awaitingResumeOk) until the player taps OK; the run is
   *  already fully restored behind it. */
  _showLostYouPopup() {
    const objs = [];
    const D = 240;
    const cx = SCREEN_W / 2, cy = SCREEN_H / 2;

    const scrim = this.add.rectangle(cx, cy, 4000, SCREEN_H + 800, 0x000000, 0.55)
      .setDepth(D).setInteractive();
    scrim.on('pointerdown', (p) => { p.event?.stopPropagation?.(); });
    objs.push(scrim);

    const cardW = 400, cardH = 184;
    const card = this.add.graphics().setDepth(D + 1);
    card.fillStyle(0x050812, 0.98);
    card.fillRoundedRect(cx - cardW / 2, cy - cardH / 2, cardW, cardH, 7);
    card.lineStyle(3, 0x39A8FF, 0.92);
    card.strokeRoundedRect(cx - cardW / 2, cy - cardH / 2, cardW, cardH, 7);
    card.lineStyle(1, 0xFF39AF, 0.70);
    ...
    const ttl = this.add.text(cx, cy - 50, 'SORRY, WE LOST YOU…', {
      fontSize: '20px', fontFamily: '"Arial Black", sans-serif', color: '#F4F7FF',
      stroke: '#FF39AF', strokeThickness: 2, align: 'center',
    }).setOrigin(0.5).setDepth(D + 2);
    objs.push(ttl);

    const msg = this.add.text(cx, cy - 8,
      'The game reloaded, but your run is safe.\nWe picked you back up right where you were.', {
      fontSize: '13px', fontFamily: '"Helvetica Neue", Arial, sans-serif',
      color: '#E3EDFF', align: 'center', wordWrap: { width: cardW - 36 },
    }).setOrigin(0.5).setDepth(D + 2);
    objs.push(msg);

    const bw = 140, bx = cx - bw / 2, by = cy + 36;
    const bg = this.add.graphics().setDepth(D + 2);
    const draw = (hover = false) => {
      bg.clear();
      bg.fillStyle(0x050812, hover ? 1 : 0.92);
      bg.fillRoundedRect(bx, by, bw, 38, 5);
      bg.lineStyle(hover ? 3 : 2, 0xFF39AF, 1);
      bg.strokeRoundedRect(bx, by, bw, 38, 5);
    };
    draw(false);
    bg.setInteractive(new Phaser.Geom.Rectangle(bx, by, bw, 38), Phaser.Geom.Rectangle.Contains);
    bg.input.cursor = 'pointer';
    const txt = this.add.text(cx, by + 19, 'OK', {
      fontSize: '17px', fontFamily: '"Arial Black", sans-serif',
      color: '#F4F7FF', stroke: '#FF39AF', strokeThickness: 2,
    }).setOrigin(0.5).setDepth(D + 3);
    bg.on('pointerover', () => draw(true));
    bg.on('pointerout', () => draw(false));
    bg.on('pointerdown', (p) => {
      p.event?.stopPropagation?.();
      objs.forEach(o => o?.destroy?.());
      this._awaitingResumeOk = false;   // unfreeze; car coasts until first steer tap
    });
    objs.push(bg, txt);
  ...
  _buildCodeEntryPopup(defaultCode, onAccept, onCancel, resumeHint = '') {
    this._modalOpen = true;
    // Portable save codes are long (~58 chars — they carry the full run),
    // so the old 5-slot keypad can't hold one.  Use a single DOM text field
    // that accepts PASTE (and typing) — the real cross-device flow: copy the
    ...
    // (reported 2026-06-18).  Top-anchored, the buttons always sit above the
    // keyboard.  overflow:auto is a fallback so a very short viewport can
    // still scroll to them.
    wrap.style.cssText = 'position:fixed;inset:0;z-index:99999;display:flex;align-items:flex-start;justify-content:center;overflow:auto;padding:max(10px,env(safe-area-inset-top)) 10px 10px;box-sizing:border-box;background:rgba(0,0,0,0.62);font-family:Arial,Helvetica,sans-serif;';
    ...
    // CSS-uppercase — that would corrupt a pasted code.  Disable autocorrect
    // too so the keyboard can't "fix" a code.
    ...
    if (resumeHint) {
      ...
      hint.style.cssText = 'color:#7FE0A8;font-size:12.5px;line-height:1.45;text-align:center;margin-top:10px;white-space:pre-line;';
    }
    const row = document.createElement('div');
    row.style.cssText = 'display:flex;gap:10px;margin-top:12px;';
    const done = () => { this._modalOpen = false; wrap.remove(); };
    // RESUME: hand the code to onAccept.  A return of {ok:false,error} means
    // the code didn't resolve — pop a tap-to-dismiss alert ON TOP of the code
    // modal (a Phaser _showPopup hid behind the pause menu) and KEEP the modal
    // open so the player can re-enter.  Anything else (ok:true) closes it.
    const submit = () => {
      const v = (input.value || '').trim();
      const res = onAccept ? onAccept(v) : { ok: true };
      if (res && res.ok === false) { showAlert(res.error || 'Invalid code.'); return; }
      done();
    };
    const mkBtn = (label, color, fn) => {
      ...
      // Press feedback — invert to the neon color while held so the player can
      // SEE the tap registered (vs. "is it the button or my phone?").
      const press   = () => { b.style.background = color;     b.style.color = '#02101f'; };
      const release = () => { b.style.background = '#050812'; b.style.color = '#F4F7FF'; };
    ...
    const showAlert = (msg) => {
      ...
      box.style.cssText = 'background:#020611;border:2px solid #FF4466;border-radius:10px;padding:18px 22px;max-width:min(86vw,420px);text-align:center;box-shadow:0 12px 34px rgba(0,0,0,.6);';
      const m = document.createElement('div');
      m.textContent = msg;
      m.style.cssText = 'color:#F4F7FF;font-size:15px;line-height:1.4;margin-bottom:14px;';
      const ok = mkBtn('OK', '#FF39AF', () => { al.remove(); try { input.focus(); } catch (_) {} });
      ok.style.flex = 'none'; ok.style.minWidth = '130px';
      box.appendChild(m); box.appendChild(ok); al.appendChild(box);
      wrap.appendChild(al);
    };
    row.appendChild(mkBtn('CANCEL', '#39A8FF', () => { done(); onCancel?.(); }));
    row.appendChild(mkBtn('RESUME', '#FF39AF', submit));
    card.appendChild(ttl); card.appendChild(input);
    if (resumeHint) card.appendChild(hint);
    card.appendChild(row);
    wrap.appendChild(card);
    document.body.appendChild(wrap);
    input.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); submit(); } });
    setTimeout(() => { try { input.focus(); input.select(); } catch (e) { /* focus best-effort */ } }, 60);
  }
  ...
  _startGameplay() {
    ...
    // frozen in the "ready" state.  Fresh runs only (resumes skip
    // _startGameplay), so it won't nag on every checkpoint.
    const _steerLabels = { classic: 'THUMBS', flappy: 'TAP', tilt: 'TILT', default: 'DEFAULT' };
    const _smPick = this._steeringMode?.() ?? 'default';
    this._showPopup?.(`🎮 ${_steerLabels[_smPick] ?? 'THUMBS'} STEERING\nHandling changes with your control type`, '#39D9FF');
  ...
  _warpToMile(mile, label = null) {
    if (!this.player || typeof mile !== 'number') return false;
    const clamped = Math.max(0, Math.min(TOTAL_ROUTE_MILES, mile));
    this.player.position = (clamped / TOTAL_ROUTE_MILES) * ROUTE_SEGS * SEG_LENGTH;
    this.player.speed    = Math.min(this.player.speed ?? 0, MAX_SPEED * 0.3);
    this.lastSegIdx      = Math.floor(this.player.position / SEG_LENGTH);
    this._odometer       = clamped;
    // Clear traffic + cops + wanted so we don't spawn mid-collision or bust.
    if (this.traffic) this.traffic.length = 0;
    ...
    this._trapStopHoldTimer = 0;
    this._trapTicket        = null;
    // Refresh the party clock so a near-finish warp still reads ON-TIME.
    this._partyClockSec = Difficulty.partyClockSec();
    this._showPopup?.(`WARP → ${label ?? `mile ${Math.round(clamped)}`}`, '#FFCC00');
    return true;
  }
  ...
  _onArrested() {
    // One bust per resolution — the strike arrest path AND a cop-ram wreck can
    // both resolve in the same frame; guard so the bail loss is only charged
    // once.  (On Easy this guard is cleared after the respawn below so the
   ...
   *  Objective specs live in DailyChallenges.js; this is the interpreter. */
  _gradeDailyObjective() {
    const t = this._dailyTracker;
    const o = t?.obj;
    ...
    switch (o.type) {
      ...
      case 'all_meters_zero_at_end':
        return allMetersZero()
          ? { pass: true,  reason: 'Stone-cold sober' }
          : { pass: false, reason: 'Still under the influence' };
      case 'combo_whole_segment':
        if (!t.comboEverActive) return { pass: false, reason: 'No combo lit' };
        return t.comboBroken
          ? { pass: false, reason: 'Combo dropped mid-run' }
          : { pass: true,  reason: 'Combo held the whole way' };
      case 'hit_cars': {
        const need = o.count ?? 1;
        return t.carsHit >= need
          ? { pass: true,  reason: `Totaled ${t.carsHit} cars` }
          : { pass: false, reason: `Only ${t.carsHit}/${need} cars` };
      }
      case 'never_starred':
        return t.maxStars < 1
          ? { pass: true,  reason: 'Never got starred' }
          : { pass: false, reason: 'They put a star on you' };
      case 'reach_stars': {
        const need = o.stars ?? 5;
        return t.maxStars >= need
      ...
      case 'no_barrier_scrape':
        ...
        if (o.noOffroad && t.offroad) return { pass: false, reason: 'Drifted off-road' };
        return { pass: true, reason: 'Held your lane' };
      case 'kill_cops': {
        const need = o.count ?? 5;
        return t.copsKilled >= need
          ? { pass: true,  reason: `Took down ${t.copsKilled} cruisers` }
          : { pass: false, reason: `Only ${t.copsKilled}/${need} cops` };
      }
      // Reaching the end city already proves these — no extra failure state
  ...
  _endGame(cause, extra = {}) {
    ...
    if (!this._statsTripEnded) {
      ...
      this._clearLiveRun();   // run is over — don't auto-resume a dead trip
